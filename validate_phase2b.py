                ...
                if response.status_code == 200:
                    ...
                    if elements:
                        ...
                        for i, element in enumerate(elements[:3]):  # Show first 3
                            print(f"   {i+1}. {element.get('tag_name', 'unknown')} - {element.get('semantic_role', 'unknown')}")
                            print(f"      Text: {element.get('text_content', '')[:50]}...")
                            print(f"      Confidence: {element.get('interaction_confidence', 0):.2f}")
                    ...
                    if capabilities:
                        ...
                        for cap in capabilities:
                            print(f"   • {cap.get('action_name', 'unknown')}: {cap.get('description', 'N/A')}")
                            print(f"     Feasibility: {cap.get('feasibility_score', 0):.2f}")
                    
                    print(f"\n🎉 PHASE 2B VALIDATION SUCCESSFUL!")
                    print(f"✅ WebAgent now has semantic 'eyes' to understand websites!")
                    return True
                else:
