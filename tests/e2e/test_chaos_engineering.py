...
class DatabaseFailureScenario(ChaosScenario):
    ...
    def __init__(self):
        super().__init__(
            "database_failure", "Simulate database disconnection and recovery"
        )
    ...
    async def start(self):
        ...
        try:
            ...
            for container in containers:
                if (
                    "postgres" in container.name.lower()
                    or "postgres" in str(container.image).lower()
                ):
...
class TestDatabaseFailures:
    ...
    async def test_database_disconnect_recovery(
        ...
        try:
            ...
            assert (
                len(server_errors) > 0
            ), "Should have server errors during database failure"
            ...
            ), "Should be able to create tasks after recovery"

            # Verify pre-chaos data is still intact
            pre_chaos_response = test_client.get(
                f"/api/v1/tasks/{pre_chaos_task_id}", headers=auth_headers["user"]
            )
    ...
    async def test_database_transaction_rollback(self, test_db, test_users_db):
        ...
        with patch("app.db.session.AsyncSession.commit") as mock_commit:
            ...
            mock_commit.side_effect = Exception(
                "Simulated database failure during commit"
            )
        ...
        assert (
            "Transaction test task" not in task_titles
        ...
        try:
            ...
            for i in range(5):
                ...
                response = test_client.get(
                    "/api/v1/tasks/", headers=auth_headers["user"]
                )
                end_time = time.time()
            ...
            success_rate = sum(1 for code in stress_responses if code == 200) / len(
                stress_responses
            )
            ...
            assert (
                success_rate >= 0.8
            ), f"Success rate {success_rate:.2%} too low under CPU stress"

            # Response times may increase but should remain reasonable
            assert (
                stress_avg < baseline_avg * 5
...
class TestNetworkFailures:
    ...
    @pytest.mark.asyncio
    @pytest.mark.e2e
    async def test_network_latency_tolerance(
        self, test_db, test_users_db, test_client, auth_headers
    ):
        """
