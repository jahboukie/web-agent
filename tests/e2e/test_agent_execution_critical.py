...
from app.models.execution_plan import AtomicAction, PlanStatus
...
class TestCriticalAgentExecution:
    ...
    async def test_complete_workflow_complex_spa(
        ...
        assert (
            len(parse_result.interactive_elements) >= 5
        ), "Should find multiple interactive elements"
        assert parse_result.page_structure is not None
        assert (
            len(parse_result.semantic_elements) > 0
        ), "Should extract semantic information"
        ...
        assert any(
            action.validation_criteria for action in plan_result.atomic_actions
        ...
        assert (
            total_duration < 60.0
        ), f"Total workflow took {total_duration:.2f}s, expected < 60s"
        ...
        success_rate = sum(
            1 for r in execution_result.action_results if r.success
        ) / len(execution_result.action_results)
        assert (
            success_rate >= 0.8
        ), f"Action success rate {success_rate:.2f} below threshold"
        ...
        assert analytics_data["reader_duration"] == pytest.approx(
            parse_duration, abs=0.5
        )
        assert analytics_data["planner_duration"] == pytest.approx(
            plan_duration, abs=0.5
        )
        assert analytics_data["actor_duration"] == pytest.approx(
            execute_duration, abs=0.5
        )
    ...
    async def test_error_recovery_reader_failure(self, test_db, test_users_db):
        ...
        with patch(
            "app.services.web_parser.web_parser_service.parse_webpage_async"
        ) as mock_parse:
            mock_parse.side_effect = TimeoutError("Network timeout")
        ...
        assert (
            "network" in result.error_type.lower() or "dns" in result.error_type.lower()
        )
    ...
    async def test_error_recovery_planner_failure(self, test_db, test_users_db):
        ...
        if plan_result.status == PlanStatus.FAILED:
            assert (
                "ambiguous" in plan_result.error_message.lower()
                or "unclear" in plan_result.error_message.lower()
            )
    ...
    @pytest.mark.asyncio
    @pytest.mark.critical
    async def test_data_integrity_across_website_types(
        self, test_db, test_users_db, test_websites
    ):
        """
        ...
        for website_type in website_types:
            ...
            assert (
                parse_result.success is True
            ), f"Parse unsuccessful for {website_type['name']}"
            assert (
                duration < 10.0
            ), f"Parse took {duration:.2f}s for {website_type['name']}, expected < 10s"

            # Validate element extraction
            total_elements = len(parse_result.interactive_elements) + len(
                parse_result.semantic_elements
            )
            assert (
                total_elements >= website_type["min_elements"]
            ), f"Found only {total_elements} elements for {website_type['name']}, expected >= {website_type['min_elements']}"

            # Validate data structure integrity
            assert isinstance(
                parse_result.page_structure, dict
            ), f"Page structure not a dict for {website_type['name']}"
            assert (
                parse_result.url == website_type["url"]
            ), f"URL mismatch for {website_type['name']}"
            assert (
                parse_result.timestamp is not None
            ...
            for element in parse_result.interactive_elements:
                assert (
                    "selector" in element
                ), f"Missing selector in element for {website_type['name']}"
                assert (
                    "type" in element
                ), f"Missing type in element for {website_type['name']}"
                assert (
                    element["selector"] is not None
                ), f"Null selector for {website_type['name']}"
        ...
        assert (
            avg_duration < 7.0
        ), f"Average parse duration {avg_duration:.2f}s too high"

        # All website types should be successfully parsed
        success_rate = sum(1 for r in results if r["success"]) / len(results)
        assert (
            success_rate == 1.0
        ), f"Website parsing success rate {success_rate:.2f} below 100%"
    ...
    async def test_concurrent_agent_execution(
        ...
        concurrent_workflows = (
            20  # Reduced for test stability, increase for load testing
        )
        ...
        assert (
            len(valid_results) == concurrent_workflows
        ...
        avg_workflow_duration = sum(r["duration"] for r in valid_results) / len(
            valid_results
        )
        assert (
            avg_workflow_duration < 20.0
        ), f"Average workflow duration {avg_workflow_duration:.2f}s too high under load"

        # Validate no significant resource conflicts
        duration_variance = max(r["duration"] for r in valid_results) - min(
            r["duration"] for r in valid_results
        )
        assert (
