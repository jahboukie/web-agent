...
class TestDashboardMetrics:
    ...
    @pytest.mark.asyncio
    @pytest.mark.e2e
    async def test_real_time_usage_counters(self, test_db, test_users_db, test_client, auth_headers):
        """
        ...
        response = test_client.get("/api/v1/analytics/dashboard", headers=auth_headers["user"])
        ...
        activities = [
            {
                ...
                "payload": {"url": "https://httpbin.org/forms/post", "force_refresh": False},
        ...
        for activity in activities:
            ...
            metrics_response = test_client.get("/api/v1/analytics/dashboard", headers=auth_headers["user"])
            assert metrics_response.status_code == 200
            updated_metrics = metrics_response.json()

            # Verify counters incremented correctly
            for metric_name, expected_increment in activity["expected_increment"].items():
        ...
        for usage_type in ["parses", "plans", "executions"]:
            ...
            if used_key in final_metrics["usage"] and limit_key in final_metrics["limits"]:
                ...
                if "usage_percentages" in final_metrics:
                    actual_percentage = final_metrics["usage_percentages"].get(percentage_key, 0)
    ...
    @pytest.mark.asyncio
    @pytest.mark.e2e
    async def test_upgrade_cta_accuracy(self, test_db, test_users_db, test_client, auth_headers):
        """
        ...
        for scenario in usage_scenarios:
            ...
            if scenario["expected_cta"]:
                ...
                if "upgrade_cta" in dashboard_data:
                    assert dashboard_data["upgrade_cta"]["show"] is False, \
...
class TestROICalculator:
    ...
    @pytest.mark.asyncio
    @pytest.mark.e2e
    async def test_roi_calculator_accuracy(self, test_db, test_users_db, test_client, auth_headers):
        """
        ...
        for scenario in roi_scenarios:
            ...
            savings_diff = abs(roi_data["monthly_savings"] - expected["monthly_savings"])
            ...
            if roi_data["roi_percentage"] > 0:
                ...
                assert roi_data["payback_period_months"] <= 12  # Should pay back within a year for positive ROI

    @pytest.mark.asyncio
    @pytest.mark.e2e
    async def test_roi_edge_cases(self, test_db, test_users_db, test_client, auth_headers):
        """
        ...
        for case in edge_cases:
            ...
            if case["expected_result"] == "error":
                ...
                assert response.status_code == 200
                roi_data = response.json()
                assert roi_data["roi_percentage"] > 1000, \
...
class TestBillingIntegration:
    ...
    async def test_stripe_billing_events_accuracy(self, test_db, test_users_db):
        ...
        usage_events = [
            {"type": "plans_used", "amount": 15, "timestamp": datetime.utcnow() - timedelta(days=25)},
            {"type": "plans_used", "amount": 20, "timestamp": datetime.utcnow() - timedelta(days=20)},
            {"type": "plans_used", "amount": 25, "timestamp": datetime.utcnow() - timedelta(days=15)},
            {"type": "executions_used", "amount": 10, "timestamp": datetime.utcnow() - timedelta(days=10)},
            {"type": "executions_used", "amount": 15, "timestamp": datetime.utcnow() - timedelta(days=5)},
            {"type": "executions_used", "amount": 20, "timestamp": datetime.utcnow() - timedelta(days=2)}
        ]
        ...
        total_plans = sum(e["amount"] for e in usage_events if e["type"] == "plans_used")  # 60
        total_executions = sum(e["amount"] for e in usage_events if e["type"] == "executions_used")  # 45
        ...
        execution_overage = max(0, total_executions - execution_limit)  # 15 executions over

        expected_overage_cost = (plan_overage * 5.00) + (execution_overage * 2.00)  # $80
        ...
        with patch('app.services.billing_service.stripe') as mock_stripe:
            ...
            billing_result = await billing_service.generate_monthly_invoice(test_db, user.id)
        ...
        base_call = next((call for call in stripe_calls if "Base subscription" in str(call)), None)
        assert base_call is not None, "Should have base subscription invoice item"

        if plan_overage > 0:
            plan_overage_call = next((call for call in stripe_calls if "plan" in str(call).lower()), None)
            assert plan_overage_call is not None, "Should have plan overage invoice item"

        if execution_overage > 0:
            exec_overage_call = next((call for call in stripe_calls if "execution" in str(call).lower()), None)
            assert exec_overage_call is not None, "Should have execution overage invoice item"
...
class TestAuditLogExports:
    ...
    @pytest.mark.asyncio
    @pytest.mark.e2e
    async def test_audit_log_export_formats(self, test_db, test_users_db, test_client, auth_headers):
        """
        ...
        test_actions = [
            ("POST", "/api/v1/tasks/", {"title": "Audit Test Task 1", "description": "Test", "goal": "Test"}),
            ("GET", "/api/v1/tasks/", None),
            ("PUT", "/api/v1/users/me", {"full_name": "Updated Admin Name"}),
            ("POST", "/api/v1/auth/logout", None),
            ("POST", "/api/v1/auth/login", {"email": user.email, "password": "TestAdmin123!"})
        ]
        ...
        for method, endpoint, payload in test_actions:
            if method == "POST":
                if payload:
                    response = test_client.post(endpoint, json=payload, headers=auth_headers["admin"])
                else:
                ...
                response = test_client.put(endpoint, json=payload, headers=auth_headers["admin"])
        ...
        assert len(csv_rows) >= len(test_actions), "Should have audit logs for all actions"
        ...
        for column in required_csv_columns:
            assert column in csv_reader.fieldnames, f"CSV missing required column: {column}"
        ...
        for row in csv_rows[-len(test_actions):]:  # Check recent entries
            ...
            assert row["action"] in [f"{method} {endpoint}" for method, endpoint, _ in test_actions]
    ...
    @pytest.mark.asyncio
    @pytest.mark.e2e
    async def test_audit_log_data_retention(self, test_db, test_users_db, test_client, auth_headers):
        """
        ...
        test_actions = [
            "login", "create_task", "view_dashboard", "update_profile", "logout"
        ]
        ...
        if post_deletion_response.status_code == 200:
            ...
            for log in post_deletion_logs:
                if log["user_id"] == user.id:
                    ...
                    assert log["user_email"] == "anonymized@deleted.user" or log["user_email"] is None
                    assert "anonymized" in log.get("details", {}).get("status", "").lower()
        else:
...
class TestDataIntegrityValidation:
    ...
    @pytest.mark.asyncio
    @pytest.mark.e2e
    async def test_cross_system_data_consistency(self, test_db, test_users_db, test_client, auth_headers):
        """
        ...
        test_sequence = [
            {
                ...
                "payload": {"title": "Consistency Test Task", "description": "Test", "goal": "Test consistency"},
                "expected_records": {
        ...
        for sequence_item in test_sequence:
            ...
            if analytics_response.status_code == 200:
                ...
                matching_events = [
                    event for event in analytics_events
            ...
            if billing_response.status_code == 200:
                ...
                if usage_type in billing_usage:
                    assert billing_usage[usage_type] > 0, \
        ...
        final_analytics = test_client.get(
            f"/api/v1/analytics/events?user_id={user.id}",
            headers=auth_headers["admin"]
        )
        ...
        if final_analytics.status_code == 200 and final_audit.status_code == 200:
            ...
            for analytics_event in analytics_events[-len(test_sequence):]:
                analytics_time = datetime.fromisoformat(analytics_event["timestamp"].replace('Z', '+00:00'))

                # Find corresponding audit log
                corresponding_audit = next(
                    (log for log in audit_logs
                     if abs((datetime.fromisoformat(log["timestamp"].replace('Z', '+00:00')) - analytics_time).total_seconds()) < 10),
                    None
                )

                if corresponding_audit:
                    audit_time = datetime.fromisoformat(corresponding_audit["timestamp"].replace('Z', '+00:00'))
