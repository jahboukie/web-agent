...
class TestPrivilegeEscalation:
    ...
    @pytest.mark.asyncio
    @pytest.mark.e2e
    async def test_api_parameter_manipulation(
        ...
        for endpoint in admin_endpoints:
            ...
            ), f"Basic user should not access {endpoint}"

            # Verify security event is logged
            error_data = response.json()
            assert (
                "privilege" in error_data["error"]["message"].lower()
                or "forbidden" in error_data["error"]["message"].lower()
            )
        ...
        for payload in malicious_payloads:
            ...
            if response.status_code == 201:
                ...
                assert (
                    task_data["user_id"] == basic_user.id
...
class TestXSSPrevention:
    ...
    @pytest.mark.asyncio
    @pytest.mark.e2e
    async def test_xss_in_user_inputs(
        self, test_db, test_users_db, test_client, auth_headers, security_test_payloads
    ):
        """
        ...
        xss_search_params = [f"search={quote(payload)}" for payload in xss_payloads]

        for search_param in xss_search_params:
            response = test_client.get(
                f"/api/v1/tasks/?{search_param}", headers=auth_headers["user"]
            )
    ...
    async def test_content_security_policy(self, test_client):
        ...
        endpoints = ["/", "/health", "/docs", "/api/v1/auth/login"]
...
class TestSQLInjectionPrevention:
    ...
    @pytest.mark.asyncio
    @pytest.mark.e2e
    async def test_sql_injection_in_queries(
        self, test_db, test_users_db, test_client, auth_headers, security_test_payloads
    ):
        """
        ...
        for payload in sql_payloads:
            # Test in task search
            response = test_client.get(
                f"/api/v1/tasks/?search={quote(payload)}", headers=auth_headers["user"]
            )
            ...
            if response.status_code == 200:
                ...
                if isinstance(tasks, list):
                    for task in tasks:
                        assert (
                            task["user_id"] == test_users_db["user"].id
        ...
        for payload in sql_payloads:
            ...
            assert response.status_code in [
                ...
                404,
                422,
            ], "SQL injection in user search should not cause server error"
    ...
    async def test_parameterized_queries(self, test_db, test_users_db):
        ...
        for dangerous_string in dangerous_strings:
            ...
            task_data = TaskCreate(
                title=dangerous_string, description="Test description", goal="Test goal"
            )

            try:
                task = await TaskService.create_task(
                    test_db, test_users_db["user"].id, task_data
                )
...
class TestRateLimiting:
    ...
    async def test_api_rate_limiting(
        ...
        assert (
            len(rate_limited_attempts) > 0
        ), "Rate limiting should be triggered after multiple failed attempts"
        ...
        avg_response_time = sum(r["response_time"] for r in rapid_requests) / len(
            rapid_requests
        )
        ...
        response_time_increase = (
            rapid_requests[-1]["response_time"] > rapid_requests[0]["response_time"] * 2
        )

        assert (
            rate_limited or response_time_increase
    ...
    async def test_cryptographic_rate_limiting(self, test_db, test_users_db):
        ...
        blocked_attempts = [
            a for a in key_generation_attempts if not a.get("allowed", True)
        ]
        assert (
            len(blocked_attempts) > 0
        ), "Cryptographic operations should be rate limited"
        ...
        blocked_encryption = [
            a for a in encryption_attempts if not a.get("allowed", True)
        ]
        assert (
            len(blocked_encryption) > 0
        ), "Encryption operations should be rate limited"
