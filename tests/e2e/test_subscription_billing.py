...
class TestSubscriptionBilling:
    ...
    @pytest.mark.asyncio
    @pytest.mark.e2e
    async def test_free_tier_usage_limits(self, test_db, test_users_db, test_client, auth_headers):
        """
    ...
    @pytest.mark.asyncio
    @pytest.mark.e2e
    async def test_plan_upgrade_downgrade_flow(self, test_db, test_users_db, test_client, auth_headers):
        """
        ...
        subscription = await subscription_service.get_user_subscription(test_db, user.id)
        assert subscription["tier"] == "reader_pro"
        assert subscription["limits"]["parses"] == 2000  # Reader Pro limit
        
        # Test feature access
        response = test_client.get(
            "/api/v1/analytics/reader",
            headers=auth_headers["user"]
        )
    ...
    @pytest.mark.asyncio
    @pytest.mark.e2e
    async def test_enterprise_tier_features(self, test_db, test_users_db, test_client, auth_headers):
        """
    ...
    async def test_billing_integration_accuracy(self, test_db, test_users_db):
        ...
        usage_events = [
            {"type": "plans_used", "amount": 25, "timestamp": datetime.utcnow() - timedelta(days=20)},
            {"type": "plans_used", "amount": 30, "timestamp": datetime.utcnow() - timedelta(days=15)},
            {"type": "executions_used", "amount": 15, "timestamp": datetime.utcnow() - timedelta(days=10)},
            {"type": "executions_used", "amount": 20, "timestamp": datetime.utcnow() - timedelta(days=5)}
        ]
        ...
        execution_overage = max(0, total_executions - execution_limit)  # 5 executions over
        
        expected_overage_cost = (plan_overage * 5.00) + (execution_overage * 2.00)  # $35
    ...
    @pytest.mark.asyncio
    @pytest.mark.e2e
    async def test_subscription_analytics_accuracy(self, test_db, test_users_db, test_client, auth_headers):
        """
        ...
        reader_rec = next((r for r in recommendations if r["target_tier"] == "reader_pro"), None)
        ...
        events = await analytics_service.get_user_events(test_db, user.id, "upgrade_cta_clicked")
        assert len(events) > 0
