...
class TestAgentExecutionFlow:
    ...
    @pytest.mark.asyncio
    @pytest.mark.e2e
    async def test_complete_workflow_simple_form(self, test_db, test_users_db, test_websites):
        """
        ...
        element_types = [elem.element_type for elem in parse_result.interactive_elements]
        for expected_type in website["expected_elements"]:
            assert any(expected_type in elem_type for elem_type in element_types), \
    ...
    @pytest.mark.asyncio
    @pytest.mark.e2e
    async def test_workflow_with_spa_application(self, test_db, test_users_db, test_websites):
        """
    ...
    @pytest.mark.asyncio
    @pytest.mark.e2e
    async def test_error_recovery_mid_workflow(self, test_db, test_users_db, test_websites):
        """
        ...
        with pytest.raises(Exception):
            await planning_service.generate_plan_async(
                ...
                planning_options={
                    "planning_timeout_seconds": 1  # Too short timeout
                },
                user_id=user.id
            )
    ...
    @pytest.mark.asyncio
    @pytest.mark.e2e
    async def test_data_integrity_across_components(self, test_db, test_users_db, test_websites):
        """
        ...
        form_elements = [
            elem for elem in parse_result.interactive_elements 
        ...
        action_selectors = [action.selector for action in planning_result.atomic_actions]
        ...
        type_actions = [
            action for action in planning_result.atomic_actions 
    ...
    @pytest.mark.asyncio
    @pytest.mark.e2e
    async def test_workflow_performance_benchmarks(self, test_db, test_users_db, test_websites):
        """
    ...
    @pytest.mark.asyncio
    @pytest.mark.e2e
    async def test_concurrent_workflow_execution(self, test_db, test_users_db, test_websites):
        """
        ...
        parse_tasks = []
        for i, (user, website) in enumerate(zip(users, websites)):
        ...
        plan_tasks = []
        for i, (user, result) in enumerate(zip(users, parse_results)):
