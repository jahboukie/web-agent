...
class TestPerformanceLoadCritical:
    ...
    async def test_concurrent_agent_execution_500_users(self, test_db, test_users_db, test_websites):
        """
        ...
        user_scenarios = [
            {"weight": 40, "scenario": "reader_heavy", "operations": ["parse", "parse", "parse"]},
            {"weight": 30, "scenario": "planner_heavy", "operations": ["parse", "plan", "plan"]},
            {"weight": 20, "scenario": "actor_heavy", "operations": ["parse", "plan", "execute"]},
            {"weight": 10, "scenario": "mixed_operations", "operations": ["parse", "plan", "execute"]}
        ]
                ...
                for operation in scenario["operations"]:
                    ...
                    if operation == "parse":
                        ...
                        if not parse_result or not parse_result.success:
                            ...
                            session_results["errors"].append(f"Parse failed for user {user_id}")
                        ...
                        session_results["operations"].append({
                            "type": "plan",
                            "duration_ms": op_duration,
                            "success": plan_result.status == "ready" if plan_result else False
                        })
                        
                        if not plan_result or plan_result.status != "ready":
                            session_results["success"] = False
                            session_results["errors"].append(f"Planning failed for user {user_id}")
                    
                    elif operation == "execute" and plan_result and plan_result.status == "ready":
                        ...
                        if not exec_result or not exec_result.success:
                            session_results["success"] = False
                            session_results["errors"].append(f"Execution failed for user {user_id}")
        ...
        avg_session_duration = statistics.mean([r["total_duration"] for r in valid_results])
        p95_session_duration = statistics.quantiles([r["total_duration"] for r in valid_results], n=20)[18]  # 95th percentile
        ...
        for op_type in ["parse", "plan", "execute"]:
            ...
            if ops:
                ...
                operation_metrics[op_type] = PerformanceMetrics(
                    ...
                    p99_response_time_ms=statistics.quantiles(durations, n=100)[98] if len(durations) > 100 else max(durations),
                    max_response_time_ms=max(durations),
                    min_response_time_ms=min(durations),
                    operations_per_second=len(ops) / load_test_duration,
                    error_rate_percent=(len(ops) - success_count) / len(ops) * 100,
                    start_time=datetime.utcnow() - timedelta(seconds=load_test_duration),
        ...
        assert avg_session_duration <= 30000, f"Average session duration {avg_session_duration:.0f}ms exceeds 30s threshold"
        assert p95_session_duration <= 60000, f"P95 session duration {p95_session_duration:.0f}ms exceeds 60s threshold"
        
        # Component-specific performance assertions
        for op_type, metrics in operation_metrics.items():
            print(f"{op_type.title()} metrics: {metrics.avg_response_time_ms:.0f}ms avg, {metrics.error_rate_percent:.1f}% error rate")
            
            if op_type == "parse":
                assert metrics.p95_response_time_ms <= 5000, f"Parse P95 latency {metrics.p95_response_time_ms:.0f}ms exceeds 5s"
                assert metrics.error_rate_percent <= 10, f"Parse error rate {metrics.error_rate_percent:.1f}% exceeds 10%"
            elif op_type == "plan":
                assert metrics.p95_response_time_ms <= 10000, f"Plan P95 latency {metrics.p95_response_time_ms:.0f}ms exceeds 10s"
                assert metrics.error_rate_percent <= 15, f"Plan error rate {metrics.error_rate_percent:.1f}% exceeds 15%"
            elif op_type == "execute":
                assert metrics.p95_response_time_ms <= 30000, f"Execute P95 latency {metrics.p95_response_time_ms:.0f}ms exceeds 30s"
                assert metrics.error_rate_percent <= 20, f"Execute error rate {metrics.error_rate_percent:.1f}% exceeds 20%"
    ...
    async def test_hsm_latency_high_throughput(self, test_db, test_users_db):
        ...
        test_key = await zero_knowledge_service.generate_user_keypair(
            user.id,
            {"algorithm": "RSA-4096", "use_hsm": True}
        )
        ...
        print(f"Starting HSM load test: {target_rps} ops/sec for {test_duration_seconds}s")
        ...
        if successful_ops:
            ...
            print(f"HSM Performance Results:")
    ...
    async def test_database_performance_under_load(self, test_db, test_users_db):
        ...
        for op_type in operation_types:
            ops = [op for op in all_operations if op["type"] == op_type and op["success"]]
            if ops:
                durations = [op["duration_ms"] for op in ops]
        ...
        for op_type, metrics in db_metrics.items():
            print(f"  {op_type}: {metrics['avg_duration_ms']:.1f}ms avg, {metrics['p95_duration_ms']:.1f}ms P95")
        
        # Database performance assertions
        assert error_rate <= 2.0, f"Database error rate {error_rate:.2f}% exceeds 2% threshold"
        assert operations_per_second >= 100, f"Database throughput {operations_per_second:.1f} ops/sec below 100 threshold"
        
        # Operation-specific assertions
        if "query_tasks" in db_metrics:
            assert db_metrics["query_tasks"]["p95_duration_ms"] <= 100, \
                   f"Query P95 latency {db_metrics['query_tasks']['p95_duration_ms']:.1f}ms exceeds 100ms"
        
        if "create_task" in db_metrics:
            assert db_metrics["create_task"]["p95_duration_ms"] <= 500, \
    ...
    async def test_auto_scaling_threshold_validation(self, test_db, test_users_db):
        ...
        for load_level in load_increments:
            ...
            # Apply load
            load_results = await self._apply_concurrent_load(test_db, test_users_db, load_level)
            
            # Monitor system resources during load
            load_metrics = await self._get_system_metrics()
            
            # Check if auto-scaling was triggered
            scaling_triggered = await self._check_auto_scaling_triggers(initial_metrics, load_metrics)
            ...
            if load_level >= 100:  # Expect scaling at higher loads
                if load_metrics["cpu_percent"] > 80 or load_metrics["memory_percent"] > 80:
        ...
        scaling_triggered_count = len([e for e in scaling_events if e["scaling_triggered"]])
        
        print(f"Auto-scaling triggered {scaling_triggered_count}/{len(scaling_events)} times")
        
        # Validate scaling responsiveness
        assert scaling_triggered_count >= 2, "Auto-scaling should trigger at high load levels"
        
        # Validate scaling is not too aggressive (shouldn't trigger at low loads)
        low_load_scaling = [e for e in scaling_events if e["load_level"] <= 25 and e["scaling_triggered"]]
        assert len(low_load_scaling) == 0, "Auto-scaling should not trigger at low loads"
    ...
    async def _validate_system_resources_under_load(self):
        ...
        assert memory.percent <= 90, f"Memory usage {memory.percent}% exceeds 90% threshold"
    ...
    async def _apply_concurrent_load(self, test_db, test_users_db, load_level: int) -> Dict[str, Any]:
        ...
        return {
            "load_level": load_level,
            "duration": duration,
            "p95_response_time": 1000 + (load_level * 5),  # Simulated response time increase
            "error_rate": min(load_level * 0.1, 5)  # Simulated error rate increase
        }
    
    async def _check_auto_scaling_triggers(self, initial_metrics: Dict, load_metrics: Dict) -> bool:
