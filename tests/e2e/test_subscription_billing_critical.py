...
class TestSubscriptionBillingCritical:
    ...
    @pytest.mark.billing
    async def test_free_tier_feature_gating_comprehensive(
        ...
        assert any(
            "approaching limit" in opp["title"].lower() for opp in upgrade_opportunities
        )
        ...
        for i in range(16):  # 80% of 20 plan limit
            await subscription_service.track_usage(test_db, user.id, "plans", 1)

        plan_response = await test_client.post(
            "/api/v1/plans/generate",
            json={"goal": "Test planning", "url": "https://example.com"},
            headers=auth_headers,
        )
        ...
        for i in range(10):  # Full execution limit
            await subscription_service.track_usage(test_db, user.id, "executions", 1)

        exec_response = await test_client.post(
            "/api/v1/execute", json={"plan_id": 123}, headers=auth_headers
        )
        ...
        conversion_data = await analytics_service.get_conversion_metrics(
            test_db, user.id
        )
        assert (
            conversion_data["upgrade_prompts_shown"] > 0
        ), "Should track upgrade prompts"
    ...
    async def test_subscription_lifecycle_management(self, test_db, test_users_db):
        ...
        assert (
            upgraded_subscription.id != free_subscription.id
        ), "Should create new subscription"

        # Old subscription should be cancelled
        old_subscription = await subscription_service.get_subscription(
            test_db, free_subscription.id
        )
        ...
        downgraded_subscription = await subscription_service.downgrade_subscription(
            ...
            user.id,
            {
                ...
                + timedelta(days=1),  # End of current period
            },
        )

        # Should schedule downgrade for end of billing period
        assert (
            downgraded_subscription.tier == SubscriptionTier.COMPLETE_PLATFORM
        ), "Current tier unchanged"
        assert (
            downgraded_subscription.scheduled_tier_change == SubscriptionTier.READER_PRO
        )
        ...
        assert (
            cancelled_subscription.status == SubscriptionStatus.ACTIVE
        ...
        with patch(
            "app.services.billing_service.stripe.PaymentIntent.create"
        ) as mock_payment:
    ...
    async def test_prorated_billing_calculations(self, test_db, test_users_db):
        ...
        with patch("app.services.billing_service.datetime") as mock_datetime:
            ...
            expected_credit = (
                old_daily_rate * days_remaining
            )  # Credit for unused Reader Pro
            expected_charge = (
                new_daily_rate * days_remaining
        ...
        expected_individual_total = (
            Decimal("129") + Decimal("179") + Decimal("229")
        )  # $537
        ...
        expected_savings_percentage = (
            expected_savings / expected_individual_total
        ) * 100  # ~25.7%

        assert abs(
            bundle_calculation.individual_total - expected_individual_total
        ) < Decimal("0.01")
        assert abs(bundle_calculation.bundle_price - expected_bundle_price) < Decimal(
            "0.01"
        )
        assert abs(bundle_calculation.savings_amount - expected_savings) < Decimal(
            "0.01"
        )
        assert abs(
            bundle_calculation.savings_percentage - expected_savings_percentage
        ) < Decimal("0.1")
        ...
        annual_calculation = await billing_service.calculate_annual_discount(
            test_db,
            user.id,
            {"tier": SubscriptionTier.COMPLETE_PLATFORM, "billing_cycle": "annual"},
        )

        monthly_price = Decimal("399")
        annual_monthly_equivalent = monthly_price * 12  # $4,788
        annual_discount_rate = Decimal("0.20")  # 20% discount
        expected_annual_price = annual_monthly_equivalent * (
            1 - annual_discount_rate
        )  # $3,830.40
        expected_annual_savings = (
            annual_monthly_equivalent - expected_annual_price
        )  # $957.60

        assert abs(annual_calculation.annual_price - expected_annual_price) < Decimal(
            "0.01"
        )
        assert abs(
            annual_calculation.annual_savings - expected_annual_savings
        ) < Decimal("0.01")
    ...
    async def test_usage_tracking_accuracy(self, test_db, test_users_db):
        ...
        expected_parses = initial_parses + len(
            [op for op in parse_operations if op.success]
        )

        assert (
            updated_usage["parses"] == expected_parses
        ...
        updated_usage = await subscription_service.get_usage_stats(test_db, user.id)
        expected_plans = initial_plans + len(
            [op for op in planning_operations if op.status == "ready"]
        )

        assert (
            updated_usage["plans"] == expected_plans
        ...
        for i, plan_op in enumerate(planning_operations):
            if plan_op and plan_op.status == "ready":
                exec_operation = await action_executor.execute_plan(
                    test_db, execution_plan=plan_op, user_id=user.id, track_usage=True
                )
        ...
        expected_executions = initial_executions + len(
            [op for op in execution_operations if op.success]
        )

        assert (
            final_usage["executions"] == expected_executions
        ...
        assert (
            historical_usage[0]["parses"] == expected_parses
    ...
    async def test_stripe_integration_comprehensive(self, test_db, test_users_db):
        ...
        with patch(
            "app.services.billing_service.stripe.PaymentMethod.attach"
        ) as mock_attach:
            ...
            payment_method = await billing_service.attach_payment_method(
                test_db,
                user.id,
                {"payment_method_id": "pm_test456", "set_as_default": True},
            )
        ...
        with patch(
            ...
            synced_subscription = (
                await subscription_service.get_subscription_by_stripe_id(
                    test_db, "sub_test789"
                )
            )
    ...
    @pytest.mark.critical
    @pytest.mark.billing
    async def test_revenue_attribution_conversion_tracking(
        self, test_db, test_users_db, test_client, auth_headers
    ):
        """
        ...
        await analytics_service.track_event(
            test_db, user.id, "payment_page_visited", {"payment_method": "stripe_card"}
        )
        ...
        conversion_rate = (
            conversion_metrics["successful_conversions"]
            / conversion_metrics["upgrade_prompts_shown"]
        ) * 100
        ...
        ab_test_results = await analytics_service.get_ab_test_results(
            test_db, {"test_name": "upgrade_prompt_messaging", "user_id": user.id}
        )

        if ab_test_results:
            assert ab_test_results["variant_shown"] in [
                "control",
                "savings_focused",
                "feature_focused",
            ]
