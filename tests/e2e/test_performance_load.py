...
class TestConcurrentAgentLoad:
    ...
    @pytest.mark.e2e
    @pytest.mark.slow
    async def test_500_concurrent_agents_peak_load(
        ...
        test_users = [test_users_db["user"], test_users_db["manager"]] * (
            concurrent_agents // 2
        )
        ...
        for coro in asyncio.as_completed(tasks):
            ...
            if completed % 50 == 0:
                ...
                print(
                    f"Progress: {completed}/{concurrent_agents} ({throughput:.2f} req/sec)"
                )
        ...
        ), f"Average response time {avg_response_time:.2f}s exceeds 30s"
        assert (
            p95_response_time < 60
        ), f"95th percentile response time {p95_response_time:.2f}s exceeds 60s"

        # Throughput analysis
        overall_throughput = len(results) / total_time
        assert (
            overall_throughput >= 10
        ), f"Throughput {overall_throughput:.2f} req/sec below 10 req/sec"
        ...
        if failed_results:
            ...
            for error_type, count in error_types.items():
                ...
                assert (
                    error_rate < 0.02
                ), f"Error type '{error_type}' rate {error_rate:.2%} exceeds 2%"
    ...
    async def test_hsm_latency_at_high_throughput(self, test_db, test_users_db):
            ...
            try:
                if operation_type == "generate_key_pair":
                    ...
                    result = await hsm_client.verify_signature(
                        data=test_data, signature=signature, key_id="signing_key_123"
                    )
        ...
        for operation, metrics in operation_metrics.items():
            ...
            p95_latency = (
                statistics.quantiles(metrics["times"], n=20)[18]
                if len(metrics["times"]) > 20
                else max(metrics["times"])
            )
        ...
        total_time = (datetime.utcnow() - start_time).total_seconds()
        actual_rps = (
            len([r for r in results if not isinstance(r, Exception)]) / total_time
        )
    ...
    @pytest.mark.asyncio
    @pytest.mark.e2e
    async def test_auto_scaling_thresholds(
        ...
        for phase_idx, phase in enumerate(traffic_phases):
            ...
            if successful_requests:
                ...
                p95_response_time = (
                    ...
                    else max([r["response_time_ms"] for r in successful_requests])
                )
            else:
            ...
            success_rate = (
                len(successful_requests) / len(phase_results) if phase_results else 0
            )
        ...
        assert (
            scale_down_phase["avg_response_time_ms"]
            <= baseline_phase["avg_response_time_ms"] * 1.5
