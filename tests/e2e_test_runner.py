...
class E2ETestRunner:
    ...
    async def run_test_suite(self, test_categories: List[str] = None) -> Dict[str, Any]:
        ...
        if test_categories:
            test_categories_to_run = {
                k: v for k, v in all_test_categories.items() 
    ...
    async def _run_test_category(self, category_name: str, config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.error(f"Test category '{category_name}' timed out after {config['timeout']}s")
            return {
                "success": False,
                "return_code": -1,
    ...
    async def _generate_test_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "critical_metrics": {
                ...
                "critical_defect_escape_rate": len(critical_failures) / total_categories,
                "mean_time_to_failure_detection": total_duration / max(failed_categories, 1)
            },
            "category_results": self.test_results,
            "critical_failures": critical_failures,
            "overall_success": len(critical_failures) == 0 and category_success_rate >= 0.95
        }
    ...
    def _generate_html_report(self, summary: Dict[str, Any]) -> str:
        ...
        for category_name, result in summary['category_results'].items():
            status_class = "category-success" if result['success'] else "category-failure"
...
async def main():
    ...
    try:
        ...
        print(f"Total Duration: {summary['execution_summary']['total_duration_formatted']}")
        print(f"Categories: {summary['category_metrics']['successful_categories']}/{summary['category_metrics']['total_categories']} passed")
        print(f"Tests: {summary['test_metrics']['passed_tests']}/{summary['test_metrics']['total_tests']} passed")
