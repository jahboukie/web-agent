...
from app.models.security import AccessSession, EnterprisePermission, EnterpriseRole
...
class TestEnterpriseIntegration:
    ...
    async def test_system_initialization(self, db_session: AsyncSession):
        ...
        result = await db_session.execute(select(func.count(EnterprisePermission.id)))
        permission_count = result.scalar()
        assert permission_count > 0, "System permissions should be created"

        # Verify roles were created
        result = await db_session.execute(select(func.count(EnterpriseRole.id)))
    ...
    async def test_sso_configuration(self, db_session: AsyncSession, test_tenant):
        ...
        test_result = await enterprise_sso_service.test_sso_configuration(
            db_session, config.id
        )
        assert "success" in test_result, "Test result should have success field"

    async def test_access_session_management(
        self, db_session: AsyncSession, test_tenant, test_user
    ):
    ...
    async def test_zero_trust_integration(self, db_session: AsyncSession, test_user):
        ...
        session = await enterprise_rbac_service.create_access_session(
            db_session, user_id=test_user.id, session_data=session_data
        )

        # Trust score should be reduced for suspicious factors
        assert (
            0.0 <= session.initial_trust_score <= 1.0
        ), "Trust score should be between 0 and 1"
        assert (
            session.initial_trust_score < 1.0
        ), "Trust score should be reduced for suspicious factors"

    async def test_compliance_framework_integration(
        self, db_session: AsyncSession, test_tenant
    ):
        """Test compliance framework integration."""
        # Verify tenant compliance settings
        assert test_tenant.compliance_level in [
            "internal",
            "confidential",
            "restricted",
            "public",
        ...
        assert (
            test_tenant.audit_retention_days >= 30
        ), "Audit retention should meet minimum requirements"
    ...
    async def test_error_handling_and_validation(self, db_session: AsyncSession):
        ...
        invalid_policy = ABACPolicyCreate(
            ...
            name="Invalid Policy",
            effect="ALLOW",
            conditions={"user.department": {"invalid_operator": "Engineering"}},
        )
