...
class WebAgentValidator:
    ...
    async def test_phase_2c_brain(self, webpage_task_id):
        ...
        async with self.session.post(
            ...
            if response.status == 200:
                ...
                # Get plan details
                async with self.session.get(
                    f"{API_BASE}/plans/{plan_id}/details", 
                    headers=self.headers
                ) as details_response:
                    if details_response.status == 200:
                        ...
                        print(f"   - Confidence score: {plan.get('confidence_score', 0):.2f}")
                        ...
                        async with self.session.post(
                            ...
                            if approval_response.status == 200:
                                ...
                                print(f"❌ Plan approval failed: {approval_response.status}")
                                return None
                    else:
                        print(f"❌ Failed to get plan details: {details_response.status}")
                        return None
            else:
    ...
    async def test_phase_2d_hands(self, plan_id):
        ...
        async with self.session.post(
            ...
            if response.status == 200:
                ...
                async with self.session.get(
                    f"{API_BASE}/execute/{execution_id}/results", 
                    headers=self.headers
                ) as results_response:
                    if results_response.status == 200:
                        results = await results_response.json()
                        ...
                        print(f"   - Steps completed: {results.get('steps_completed', 0)}/{results.get('total_steps', 0)}")
                        print(f"   - Success rate: {results.get('success_rate', 0):.1f}%")
                        print(f"   - Duration: {results.get('total_duration_seconds', 0):.1f}s")
                        print(f"   - Screenshots: {len(results.get('screenshots', []))}")
                        return execution_id
                    else:
                        print(f"❌ Failed to get execution results: {results_response.status}")
                        return None
            else:
    ...
    async def _monitor_execution_progress(self, execution_id):
        ...
        for attempt in range(60):  # 60 attempts = 2 minutes max
            ...
            async with self.session.get(
                f"{API_BASE}/execute/{execution_id}", 
                headers=self.headers
            ) as response:
                if response.status == 200:
                    data = await response.json()
                    ...
                    if status in ["completed", "failed", "cancelled"]:
                        ...
                        print(f"⏳ Execution progress: {progress}% (Step {current_step}/{total_steps})")
...
async def main():
    ...
    async with WebAgentValidator() as validator:
        ...
        print(f"{'✅' if webhook_success else '❌'} Webhook System - {'WORKING' if webhook_success else 'FAILED'}")
        print()
