...
class BrowserPoolManager:
    ...
    async def acquire_context(self, task_id: int) -> BrowserContext:
        ...
        try:
            ...
            try:
                ...
                logger.warning("Created new context due to pool exhaustion", task_id=task_id)
    ...
    async def release_context(self, task_id: int, context: BrowserContext):
        ...
        try:
            ...
            age_minutes = (datetime.utcnow() - context_info['created_at']).total_seconds() / 60
            should_cleanup = (
                age_minutes > self.max_context_age_minutes or
                context_info['usage_count'] > 50 or
                await self._check_memory_usage(context_info)
            )
            ...
            if should_cleanup:
                ...
                try:
                    ...
                    logger.debug("Context returned to pool", context_id=context_info['id'])
                except asyncio.QueueFull:
                    # Pool is full, cleanup this context
                    await self._cleanup_context(context_info)
                    logger.debug("Pool full, context cleaned up", context_id=context_info['id'])
    ...
    async def _reset_context(self, context_info: Dict[str, Any]):
        ...
        try:
            ...
            logger.error("Failed to reset context", context_id=context_info['id'], error=str(e))
            raise
    ...
    async def _cleanup_context(self, context_info: Dict[str, Any]):
        ...
        try:
            ...
            logger.error("Failed to cleanup context", context_id=context_info['id'], error=str(e))
        ...
        while True:
            try:
                ...
                for ctx_id, ctx_info in list(self.active_contexts.items()):
                    age_minutes = (current_time - ctx_info['last_used_at']).total_seconds() / 60
                    if age_minutes > self.max_context_age_minutes * 2:  # Double the normal age limit
                        stale_contexts.append(ctx_info)

                # Cleanup stale contexts
                for ctx_info in stale_contexts:
                    await self._cleanup_context(ctx_info)
                    logger.warning("Cleaned up stale active context", context_id=ctx_info['id'])
                ...
                while not self.available_contexts.empty():
                    try:
                        ...
                        age_minutes = (current_time - ctx_info['created_at']).total_seconds() / 60

                        if age_minutes <= self.max_context_age_minutes:
                            available_contexts.append(ctx_info)
                        else:
                            await self._cleanup_context(ctx_info)
                            logger.debug("Cleaned up aged available context", context_id=ctx_info['id'])
