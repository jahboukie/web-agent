...
from app.schemas.user import SecurityRole, ThreatLevel, AccessContext
...
class RBACEngine:
    ...
    def _initialize_system_roles(self) -> Dict[str, Role]:
        ...
        roles["tenant_admin"] = Role(
            ...
            permissions={
                ...
                Permission.TASKS_READ_ALL, Permission.TASKS_UPDATE_ALL, Permission.TASKS_APPROVE,
                Permission.PLANS_READ_ALL, Permission.PLANS_APPROVE, Permission.PLANS_VALIDATE,
                Permission.CREDENTIALS_READ_ALL, Permission.CREDENTIALS_UPDATE_ALL,
                Permission.SECURITY_POLICIES_READ, Permission.SECURITY_POLICIES_WRITE,
                Permission.COMPLIANCE_READ, Permission.COMPLIANCE_REPORTS_GENERATE,
        ...
        roles["automation_manager"] = Role(
            ...
            permissions={
                Permission.TASKS_CREATE, Permission.TASKS_READ_ALL, Permission.TASKS_UPDATE_ALL,
                Permission.TASKS_EXECUTE, Permission.TASKS_APPROVE,
                Permission.PLANS_CREATE, Permission.PLANS_READ_ALL, Permission.PLANS_UPDATE_ALL,
                Permission.PLANS_APPROVE, Permission.PLANS_VALIDATE,
                Permission.CREDENTIALS_CREATE, Permission.CREDENTIALS_READ_OWN, Permission.CREDENTIALS_UPDATE_OWN,
        ...
        roles["end_user"] = Role(
            ...
            permissions={
                Permission.TASKS_CREATE, Permission.TASKS_READ_OWN, Permission.TASKS_UPDATE_OWN,
                Permission.TASKS_DELETE_OWN, Permission.TASKS_EXECUTE,
                Permission.PLANS_CREATE, Permission.PLANS_READ_OWN, Permission.PLANS_UPDATE_OWN,
                Permission.PLANS_DELETE_OWN,
                Permission.CREDENTIALS_CREATE, Permission.CREDENTIALS_READ_OWN,
                Permission.CREDENTIALS_UPDATE_OWN, Permission.CREDENTIALS_DELETE_OWN,
                Permission.API_AUTOMATION
            },
            tenant_scoped=True,
            risk_level=ThreatLevel.LOW,
    ...
    async def assign_role(self, user_id: int, role_id: str, assigned_by: int, tenant_id: Optional[str] = None) -> bool:
        ...
        try:
            ...
            if not await self._can_assign_role(assigned_by, role_id, tenant_id):
                logger.warning(f"User {assigned_by} cannot assign role {role_id} to user {user_id}")
                return False
    ...
    async def check_permission(
        ...
        try:
            ...
            if (resource_id and Permission.USER_OWN_RESOURCES in user_permissions and
                await self._is_resource_owner(user_id, resource_id)):
                return True
    ...
    async def get_user_permissions(self, user_id: int, tenant_id: Optional[str] = None) -> Set[Permission]:
        ...
        try:
            ...
            for assignment in user_role_assignments:
                ...
                if role_id in self.roles:
                    ...
                    inherited_permissions = await self._get_inherited_permissions(role_id, tenant_id)
    ...
    async def _get_inherited_permissions(self, role_id: str, tenant_id: Optional[str] = None) -> Set[Permission]:
            ...
            if role_id in self.role_hierarchy:
                for parent_role_id in self.role_hierarchy[role_id]:
                    if parent_role_id in self.roles:
                        ...
                        parent_inherited = await self._get_inherited_permissions(parent_role_id, tenant_id)
    ...
    async def _can_assign_role(self, assigner_id: int, role_id: str, tenant_id: Optional[str] = None) -> bool:
        ...
        if (tenant_id and 
            await self.check_permission(assigner_id, Permission.USERS_MANAGE_ROLES, tenant_id=tenant_id)):
...
class ABACEngine:
    ...
    def _initialize_policies(self) -> Dict[str, PolicyRule]:
        ...
        policies["resource_owner_access"] = PolicyRule(
            ...
            conditions={
                "user.id": {"eq_attr": "resource.owner"}
            },
            effect="ALLOW",
            priority=100
        )
    ...
    async def _evaluate_policy_conditions(self, policy: PolicyRule, request: AccessRequest) -> bool:
        ...
        try:
            ...
            all_attributes = {
                **request.attributes,
                ...
                "user.trust_score": request.context.risk_score * 100,  # Convert to 0-100 scale
                "env.device_trust": self._get_device_trust_level(request.context.device_info),
            ...
            for attr_name, condition in policy.conditions.items():
                ...
                if not await self._evaluate_condition(attr_value, condition, all_attributes):
                    return False
    ...
    async def _evaluate_condition(self, value: Any, condition: Dict[str, Any], all_attributes: Dict[str, Any]) -> bool:
...
class EnterpriseAccessControl:
    ...
    async def check_access(
        ...
        try:
            ...
            if cache_key in self.access_cache:
                ...
                if (datetime.utcnow() - cached_decision.decision_time).seconds < 300:  # 5 min cache
            ...
            final_decision = await self._combine_decisions(rbac_allowed, abac_decision, request)
    ...
    async def _check_rbac_access(self, request: AccessRequest) -> bool:
        ...
        permission_map = {
            ...
            (ResourceType.EXECUTION_PLAN, ActionType.UPDATE): Permission.PLANS_UPDATE_OWN,
            (ResourceType.EXECUTION_PLAN, ActionType.DELETE): Permission.PLANS_DELETE_OWN,
            ...
            (ResourceType.CREDENTIAL, ActionType.UPDATE): Permission.CREDENTIALS_UPDATE_OWN,
            (ResourceType.CREDENTIAL, ActionType.DELETE): Permission.CREDENTIALS_DELETE_OWN,
            (ResourceType.AUDIT_LOG, ActionType.READ): Permission.AUDIT_LOGS_READ,
            (ResourceType.COMPLIANCE_REPORT, ActionType.READ): Permission.COMPLIANCE_READ,
        }
    ...
    async def _combine_decisions(self, rbac_allowed: bool, abac_decision: AccessDecision, request: AccessRequest) -> AccessDecision:
    ...
    async def _audit_access_decision(self, request: AccessRequest, decision: AccessDecision) -> None:
...
def require_permission(permission: Permission):
    ...
    def decorator(func):
        ...
        async def wrapper(*args, **kwargs):
            ...
            # Check permission
            has_permission = await enterprise_access_control.rbac_engine.check_permission(
                user_id, permission
            )
