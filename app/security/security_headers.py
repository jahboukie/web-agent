...
class SecurityHeadersConfig:
    ...
    def get_csp_policy(self, nonce: Optional[str] = None) -> CSPDirective:
        ...
        if self.environment == 'production':
            ...
            if nonce:
                ...
                csp.style_src(["'self'", f"'nonce-{nonce}'", "'unsafe-inline'"])  # unsafe-inline needed for some CSS frameworks
            else:
                csp.script_src(["'self'"])
            ...
            csp.script_src(["'self'", "'unsafe-inline'", "'unsafe-eval'", "localhost:*", "127.0.0.1:*"])
            csp.style_src(["'self'", "'unsafe-inline'", "localhost:*", "127.0.0.1:*"])
            csp.img_src(["'self'", "data:", "http:", "https:"])
            csp.connect_src(["'self'", "http:", "https:", "ws:", "wss:", "localhost:*", "127.0.0.1:*"])
...
class SecurityHeadersMiddleware(BaseHTTPMiddleware):
    ...
    def get_security_headers(self, request: Request, nonce: Optional[str] = None) -> Dict[str, str]:
...
class CSPReportHandler:
    ...
    async def _analyze_violation(self, violation: Dict[str, Any]) -> None:
        ...
        try:
            ...
            suspicious_patterns = [
                ...
                'malicious', 'attack', 'xss', 'injection'
            ]
            
            is_suspicious = any(pattern in blocked_uri for pattern in suspicious_patterns)
    ...
    def get_violation_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "top_blocked_uris": sorted(uri_counts.items(), key=lambda x: x[1], reverse=True)[:5],
