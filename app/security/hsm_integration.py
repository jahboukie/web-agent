...
class KeyMetadata:
    ...
    # Compliance
    fips_140_2_level: int | None = None
    common_criteria_level: str | None = None
...
class HSMInterface:
    ...
    async def generate_key(
        self, key_type: KeyType, key_usage: list[KeyUsage], **kwargs
    ) -> str:
    ...
    async def verify(
        self, key_id: str, data: bytes, signature: bytes, **kwargs
    ) -> bool:
        """Verify signature using HSM key."""
        raise NotImplementedError

    async def derive_key(
        self, master_key_id: str, derivation_data: bytes, **kwargs
    ) -> str:
        ...
        try:
            ...
            if alias:
                self.kms_client.create_alias(
                    AliasName=f"alias/webagent-{alias}", TargetKeyId=key_id
                )
    ...
    async def encrypt(self, key_id: str, plaintext: bytes, **kwargs) -> bytes:
        ...
        try:
            ...
            response = self.kms_client.encrypt(
                KeyId=key_id, Plaintext=plaintext, EncryptionContext=encryption_context
            )
...
class GoogleCloudKMSInterface(HSMInterface):
    ...
    async def generate_key(
        ...
        try:
            ...
            key_ring_path = self.client.key_ring_path(
...
class SoftwareHSMInterface(HSMInterface):
    ...
    async def generate_key(
        ...
        try:
            ...
            logger.info(
                "Key generated in software HSM", key_id=key_id, key_type=key_type.value
            )
...
class EnterpriseKeyManager:
    ...
    def _initialize_hsm_interface(self) -> HSMInterface:
        ...
        if provider == HSMProvider.AWS_CLOUDHSM:
            ...
            logger.warning(
                f"Unsupported HSM provider: {provider}, falling back to software"
            )
            return SoftwareHSMInterface()
    ...
    async def generate_master_key(
        ...
        try:
            ...
            metadata = KeyMetadata(
                ...
                audit_required=True,
                tags={"key_class": "master", "tenant_id": tenant_id or "global"},
            )
            ...
            await self._log_key_operation(
                operation_type="generate", key_id=key_id, success=True
            )
    ...
    async def derive_data_encryption_key(
        self, master_key_id: str, purpose: str, key_type: KeyType = KeyType.AES_256
    ) -> str:
        ...
        try:
            ...
            derivation_data = (
                f"{purpose}:{key_type.value}:{datetime.utcnow().isoformat()}".encode()
            )
            ...
            # Log operation
            await self._log_key_operation(
                operation_type="derive", key_id=key_id, success=True
            )
    ...
    async def encrypt_data(
        ...
        try:
            ...
            if not await self._check_key_usage_authorization(
                key_id, user_id, "encrypt"
            ):
    ...
    async def rotate_key(self, key_id: str) -> str:
        ...
        try:
            ...
            # Log operation
            await self._log_key_operation(
                operation_type="rotate", key_id=key_id, success=True
            )

            logger.info(
                "Key rotated successfully", old_key_id=key_id, new_key_id=new_key_id
            )
    ...
    async def _check_key_usage_authorization(
        self, key_id: str, user_id: int | None, operation: str
    ) -> bool:
    ...
    def _get_fips_level(self) -> int | None:
