...
class PyUpSecurityDB:
                        ...
                        async with session.get(url) as response:
                            if response.status == 200:
                                ...
                                for vuln_data in data:
                                    ...
                                    if self._is_version_affected(package.version, vuln_data.get('affected_versions', [])):
                                        vulnerability = Vulnerability(
                                            ...
                                            published_date=self._parse_date(vuln_data.get('published_date')),
                                            source="pyup.io"
                                        )
                                        vulnerabilities.append(vulnerability)
                    
                    except Exception as e:
                        logger.warning(f"Failed to check {package.name} against PyUp: {str(e)}")
                        continue
    ...
    def _is_version_affected(self, installed_version: str, affected_versions: List[str]) -> bool:
            ...
            async with aiohttp.ClientSession(timeout=aiohttp.ClientTimeout(total=self.timeout)) as session:
                for package in packages:
                    try:
                        ...
                        async with session.post(self.api_url, json=query_data) as response:
                            if response.status == 200:
                                data = await response.json()
                                ...
                                for vuln_data in data.get('vulns', []):
                                    vulnerability = Vulnerability(
                                        ...
                                        title=vuln_data.get('summary', 'Security vulnerability'),
                        ...
                        logger.warning(f"Failed to check {package.name} against OSV: {str(e)}")
                        continue
...
class DependencyScanner:
    ...
    def _deduplicate_vulnerabilities(self, vulnerabilities: List[Vulnerability]) -> List[Vulnerability]:
        ...
        for vuln in vulnerabilities:
            ...
            if key not in seen:
                ...
                existing_vuln = next(v for v in unique_vulnerabilities if f"{v.package_name}:{v.id}" == key)
    ...
    def _generate_recommendations(self, scan_result: ScanResult) -> List[str]:
        ...
        recommendations.append(
            "Run dependency scans regularly (weekly recommended)."
        )
...
def get_dependency_scanner() -> Optional[DependencyScanner]:
