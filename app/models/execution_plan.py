...
class ExecutionPlan(Base):
    ...
    requires_sensitive_actions: Mapped[bool | None] = mapped_column(Boolean, default=False)
    ...
    validation_warnings: Mapped[dict[str, Any] | None] = mapped_column(JSON, nullable=True)
    ...
    status: Mapped[PlanStatus] = mapped_column(SQLEnum(PlanStatus), default=PlanStatus.DRAFT, index=True)
    ...
    similar_plans_referenced: Mapped[dict[str, Any] | None] = mapped_column(JSON, nullable=True)
    ...
    error_recovery_strategy: Mapped[str | None] = mapped_column(String(100), nullable=True)

    task: Mapped["Task"] = relationship(back_populates="execution_plans")
    user: Mapped["User"] = relationship()
    atomic_actions: Mapped[list["AtomicAction"]] = relationship(back_populates="execution_plan", cascade="all, delete-orphan", order_by="AtomicAction.step_number")

class AtomicAction(Base):
    __tablename__ = "atomic_actions"

    id: Mapped[int] = mapped_column(primary_key=True, index=True)
    execution_plan_id: Mapped[int] = mapped_column(ForeignKey("execution_plans.id"), index=True)
    target_element_id: Mapped[int | None] = mapped_column(ForeignKey("interactive_elements.id"), nullable=True)
    ...
    element_attributes: Mapped[dict[str, Any] | None] = mapped_column(JSON, nullable=True)
    ...
    validation_criteria: Mapped[dict[str, Any] | None] = mapped_column(JSON, nullable=True)
    preconditions: Mapped[dict[str, Any] | None] = mapped_column(JSON, nullable=True)
    required_page_elements: Mapped[dict[str, Any] | None] = mapped_column(JSON, nullable=True)
    required_page_state: Mapped[dict[str, Any] | None] = mapped_column(JSON, nullable=True)
    ...
    conditional_logic: Mapped[dict[str, Any] | None] = mapped_column(JSON, nullable=True)
    skip_if_conditions: Mapped[dict[str, Any] | None] = mapped_column(JSON, nullable=True)
    ...
    status: Mapped[StepStatus] = mapped_column(SQLEnum(StepStatus), default=StepStatus.PENDING, index=True)
    ...
    before_screenshot_path: Mapped[str | None] = mapped_column(String(500), nullable=True)
    after_screenshot_path: Mapped[str | None] = mapped_column(String(500), nullable=True)
...
class PlanTemplate(Base):
    ...
    created_from_plans: Mapped[dict[str, Any] | None] = mapped_column(JSON, nullable=True)
