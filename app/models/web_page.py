...
class WebPage(Base):
    ...
    last_accessed: Mapped[datetime | None] = mapped_column(DateTime(timezone=True), onupdate=func.now())
    ...
    # Relationships
    interactive_elements: Mapped[list["InteractiveElement"]] = relationship(back_populates="web_page", cascade="all, delete-orphan")
    content_blocks: Mapped[list["ContentBlock"]] = relationship(back_populates="web_page", cascade="all, delete-orphan")
    action_capabilities: Mapped[list["ActionCapability"]] = relationship(back_populates="web_page", cascade="all, delete-orphan")
    browser_sessions: Mapped[list["BrowserSession"]] = relationship(back_populates="current_page")
