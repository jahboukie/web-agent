...
from app.security.zero_trust import ZeroTrustPolicy, zero_trust_engine
...
async def get_zero_trust_policies(current_user: User = Depends(get_current_user)):
    ...
    try:
        ...
        for policy in ZeroTrustPolicy:
            policy_info = {
                ...
                "trust_requirements": {
                    ZeroTrustPolicy.CRITICAL_SYSTEMS: {
                        "min_trust_score": 0.99,
                        "required_mfa": True,
                        "device_managed": True,
                    },
                    ZeroTrustPolicy.SENSITIVE_DATA: {
                        "min_trust_score": 0.80,
                        "required_mfa": True,
                        "device_registered": True,
                    },
                    ZeroTrustPolicy.STANDARD_ACCESS: {
                        "min_trust_score": 0.60,
                        "required_mfa": False,
                        "device_registered": False,
                    },
                    ZeroTrustPolicy.PUBLIC_ACCESS: {
...
async def get_siem_status(current_user: User = Depends(get_current_user)):
...
async def search_siem_events(
    ...
    provider: str | None = Query(default=None, description="Specific SIEM provider"),
    current_user: User = Depends(get_current_user),
):
