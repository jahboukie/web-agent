...
@router.post("/tenants", response_model=EnterpriseTenant, status_code=status.HTTP_201_CREATED)
...
async def create_tenant(
    ...
    try:
        tenant = await enterprise_tenant_service.create_tenant(db, tenant_data, current_user.id)
        return tenant
...
async def list_tenants(
    ...
    compliance_level: Optional[str] = Query(None),
    db: AsyncSession = Depends(get_db),
    current_user: User = Depends(get_current_user)
):
    """List enterprise tenants with filtering."""
    tenants = await enterprise_tenant_service.list_tenants(
        db, skip=skip, limit=limit, is_active=is_active, compliance_level=compliance_level
    )
    return tenants
...
@router.post("/roles", response_model=EnterpriseRole, status_code=status.HTTP_201_CREATED)
...
async def assign_user_roles(
    ...
    try:
        ...
        success = await enterprise_rbac_service.assign_user_roles(db, assignment, current_user.id)
        if success:
...
async def get_user_permissions(
    ...
    permissions = await enterprise_rbac_service.get_user_permissions(db, user_id, tenant_id)
    return {"user_id": user_id, "tenant_id": tenant_id, "permissions": list(permissions)}
...
async def list_permissions(
    ...
    query = query.offset(skip).limit(limit).order_by(PermissionModel.category, PermissionModel.name)
...
@router.post("/tenants/{tenant_id}/sso", response_model=SSOConfiguration, status_code=status.HTTP_201_CREATED)
@require_permission(Permission.SYSTEM_CONFIGURE)
async def create_sso_configuration(
    ...
    try:
        ...
        config = await enterprise_sso_service.create_sso_configuration(db, config_data, current_user.id)
        return config
    except ValueError as e:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=str(e))
    except Exception as e:
...
@router.post("/policies", response_model=ABACPolicy, status_code=status.HTTP_201_CREATED)
...
async def create_abac_policy(
    ...
    """Create ABAC policy."""
    try:
        policy = await enterprise_abac_service.create_policy(db, policy_data, current_user.id)
        return policy
    except ValueError as e:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=str(e))
    except Exception as e:
