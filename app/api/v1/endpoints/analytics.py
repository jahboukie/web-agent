...
async def get_analytics_dashboard(
    ...
    try:
        ...
        logger.error(
            "Failed to load analytics dashboard", user_id=current_user.id, error=str(e)
        )
        raise HTTPException(
...
async def get_usage_metrics(
    ...
    try:
        ...
        logger.error(
            "Failed to get usage metrics", user_id=current_user.id, error=str(e)
        )
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
async def get_subscription_details(
    ...
    try:
        ...
        logger.error(
            "Failed to get subscription details", user_id=current_user.id, error=str(e)
        )
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
async def get_upgrade_opportunities(
    ...
    try:
        ...
        logger.error(
            "Failed to get upgrade opportunities", user_id=current_user.id, error=str(e)
        )
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
async def get_roi_calculation(
    ...
    try:
        roi = await analytics_service.calculate_roi(db, current_user.id, hourly_rate)

        return roi

    except Exception as e:
        logger.error("Failed to calculate ROI", user_id=current_user.id, error=str(e))
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
async def get_success_metrics(
    ...
    try:
        metrics = await analytics_service.get_success_metrics(db, current_user.id)

        return metrics

    except Exception as e:
        logger.error(
            "Failed to get success metrics", user_id=current_user.id, error=str(e)
        )
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
async def get_billing_info(
    ...
    try:
        billing_info = await billing_service.get_billing_info(db, current_user.id)

        return billing_info

    except Exception as e:
        logger.error(
            "Failed to get billing info", user_id=current_user.id, error=str(e)
        )
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
async def track_analytics_event(
    ...
    try:
        await analytics_service.track_event(db, current_user.id, event_type, event_data)
