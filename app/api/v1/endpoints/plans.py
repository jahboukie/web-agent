...
from app.models.execution_plan import ExecutionPlan, PlanStatus, PlanTemplate
...
async def _process_plan_generation_async(
    task_id: int, user_goal: str, planning_options: dict[str, Any], user_id: int
):
    """Async background task function for AI plan generation."""

    logger.info(
        "🧠 AI PLAN GENERATION STARTED", task_id=task_id, user_goal=user_goal[:100]
    )
...
def _process_plan_generation(
    ...
    logger.info(
        "🎯 PLAN GENERATION SYNC WRAPPER", task_id=task_id, user_goal=user_goal[:50]
    )
    ...
    try:
        ...
        loop.run_until_complete(
            _process_plan_generation_async(
                task_id, user_goal, planning_options, user_id
            )
        )
        ...
        logger.error(
            "💥 PLAN GENERATION BACKGROUND TASK FAILED", task_id=task_id, error=str(e)
        )
...
async def generate_execution_plan(
    ...
    try:
        ...
        logger.error(
            "Failed to queue AI plan generation",
            error=str(e),
...
async def approve_execution_plan(
    ...
    try:
        ...
        if approval_request.approval_decision == "approve":
            ...
            next_action_url = "/api/v1/plans/generate"  # Generate new plan
...
async def validate_execution_plan(
    ...
    try:
        ...
        execution_plan.validation_warnings = validation_result.get("findings", {}).get(
            "warnings", []
        )
        execution_plan.validation_errors = validation_result.get("findings", {}).get(
            "critical_issues", []
        )
...
async def get_planning_metrics(
    ...
    try:
        metrics = await planning_service.get_planning_metrics(
...
def _get_status_message(
    task_status: dict[str, Any], execution_plan: ExecutionPlan | None
) -> str:
