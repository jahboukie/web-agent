...
class TaskStatusService:
    ...
    async def complete_task(
        ...
        try:
            ...
            logger.info(
                ...
                task_id=task_id,
                has_start_time=processing_started_at is not None,
            )

            actual_duration = None
            if processing_started_at:
                actual_duration = int(
                    (completion_time - processing_started_at).total_seconds()
                )
    ...
    async def fail_task(
        ...
        try:
            ...
            if not task:
                logger.error("Task not found for failure reporting", task_id=task_id)
                return False
            ...
            if not should_retry:
                ...
                if task.processing_started_at:
                    duration = int(
                        (failure_time - task.processing_started_at).total_seconds()
                    )
    ...
    async def get_task_status(
        ...
        try:
            ...
            return {
                ...
                "current_step": progress_details.get("current_step"),
