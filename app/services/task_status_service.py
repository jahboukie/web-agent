...
class TaskStatusService:
    ...
    async def complete_task(
        ...
        try:
            ...
            if task_times and task_times.processing_started_at:
                actual_duration = int((completion_time - task_times.processing_started_at).total_seconds())
    ...
    async def fail_task(
        ...
        try:
            ...
            result = await db.execute(
                select(Task.retry_count, Task.max_retries, Task.processing_started_at)
                .where(Task.id == task_id)
            )
            ...
            if not should_retry:
                ...
                if task_info and task_info.processing_started_at:
                    duration = int((failure_time - task_info.processing_started_at).total_seconds())
    ...
    async def get_task_status(
        ...
        try:
            ...
            return {
                ...
                "current_step": task.progress_details.get("current_step") if task.progress_details else None,
                ...
                "result_data": task.result_data if task.status == TaskStatus.COMPLETED else None
            }
    ...
    async def get_task_metrics(
        ...
        try:
            ...
            completed_durations = [
                t.actual_duration_seconds for t in tasks
