...
from app.models.security import SSOConfiguration, user_tenant_roles
from app.models.user import User
from app.schemas.enterprise import SSOConfigurationCreate
from app.security.sso_integration import SSOAuthResult, SSOUser
...
class EnterpriseSSOManagedService:
    ...
    async def create_sso_configuration(
        self, db: AsyncSession, config_data: SSOConfigurationCreate, created_by: int
    ) -> SSOConfiguration:
        """Create a new SSO configuration for tenant."""

        try:
            # Verify tenant exists
            tenant = await enterprise_tenant_service.get_tenant(
                db, tenant_id=config_data.tenant_id
            )
            if not tenant:
            ...
            if existing_result.scalar_one_or_none():
                raise ValueError(
                    f"SSO configuration for {config_data.provider.value} already exists for this tenant"
                )

            # Encrypt sensitive configuration data
            encryption_key_id = f"sso_key_{secrets.token_hex(8)}"
            encrypted_config = await self._encrypt_sso_config(
                config_data.configuration, encryption_key_id
            )
            ...
            if config_data.is_primary:
                await db.execute(
                    SSOConfiguration.__table__.update()
                    .where(SSOConfiguration.tenant_id == config_data.tenant_id)
            ...
            logger.info(
                f"Created SSO configuration for {config_data.provider.value} in tenant {config_data.tenant_id}"
            )
            return db_config
    ...
    async def list_sso_configurations(
        ...
        try:
            result = await db.execute(
                ...
                .where(SSOConfiguration.tenant_id == tenant_id)
    ...
    async def authenticate_sso_user(
        ...
        try:
            if not sso_auth_result.success or not sso_auth_result.user:
                return {
                    "success": False,
                    "error": sso_auth_result.error_message
                    or "SSO authentication failed",
                }
    ...
    async def _find_or_create_user(
        ...
        try:
            ...
            if user:
                ...
                await self._ensure_user_tenant_association(
                    db, user.id, tenant_id, sso_config
                )
                ...
                # Create new user
                return await self._create_user_from_sso(
                    db, sso_user, sso_config, tenant_id
                )

            else:
                logger.warning(
                    f"User {sso_user.email} not found and auto-provisioning disabled"
                )
                return None
    ...
    async def _create_user_from_sso(
        ...
        try:
            ...
            logger.info(
                f"Created user {user.email} from SSO provider {sso_config.provider}"
            )
            return user
    ...
    async def _update_user_from_sso(
        ...
        try:
            ...
            if sso_user.first_name or sso_user.last_name:
                user.full_name = (
                    f"{sso_user.first_name or ''} {sso_user.last_name or ''}".strip()
                )
            ...
            logger.info(
                f"Updated user {user.email} from SSO provider {sso_config.provider}"
            )
        ...
        try:
            ...
            if not existing_association:
                ...
                if default_role_id:
                    ...
                    logger.info(
                        f"Associated user {user_id} with tenant {tenant_id} and role {default_role_id}"
                    )
    ...
    async def _get_default_role_id(
        self, db: AsyncSession, sso_config: SSOConfiguration
    ) -> int | None:
    ...
    async def _encrypt_sso_config(
        self, config: dict[str, Any], key_id: str
    ) -> dict[str, Any]:
    ...
    async def _decrypt_sso_config(
        self, encrypted_config: dict[str, Any], key_id: str
    ) -> dict[str, Any]:
    ...
    async def test_sso_configuration(
        ...
        try:
            ...
            decrypted_config = await self._decrypt_sso_config(
                config.configuration, config.encryption_key_id
            )
            ...
            if config.protocol == "oidc":
                ...
                return {
                    "success": False,
                    "error": f"Unsupported protocol: {config.protocol}",
                }
                ...
                async with aiohttp.ClientSession() as session:
                    async with session.get(discovery_url) as response:
                        if response.status == 200:
                            return {
                                "success": True,
                                "message": "OIDC discovery endpoint accessible",
                            }
                        else:
                            return {
                                "success": False,
                                "error": f"Discovery endpoint returned {response.status}",
                            }
                ...
                async with aiohttp.ClientSession() as session:
                    async with session.get(auth_endpoint) as response:
                        if response.status in [
                            ...
                            return {
                                "success": True,
                                "message": "OIDC authorization endpoint accessible",
                            }
                        else:
                            return {
                                "success": False,
                                "error": f"Authorization endpoint returned {response.status}",
                            }
                    ...
                    async with session.get(metadata_url) as response:
                        if response.status == 200:
                            ...
                            if "EntityDescriptor" in content:
                                return {
                                    "success": True,
                                    "message": "SAML metadata accessible and valid",
                                }
                            else:
                                return {
                                    "success": False,
                            ...
                            return {
                                "success": False,
                                "error": f"Metadata endpoint returned {response.status}",
                            }
                ...
                async with aiohttp.ClientSession() as session:
                    async with session.get(sso_url) as response:
                        if response.status in [
                            ...
                            return {
                                "success": True,
                                "message": "SAML SSO endpoint accessible",
                            }
                        else:
                            return {
                                "success": False,
                                "error": f"SSO endpoint returned {response.status}",
                            }
