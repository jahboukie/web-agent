...
class WebhookService:
    ...
    async def initialize(self):
        ...
        if not http_client_manager.is_initialized:
            raise RuntimeError(
                "HTTP client manager must be initialized before webhook service"
            )
    ...
    async def send_execution_progress_webhook(
        ...
        try:
            ...
            for url in webhook_urls:
                if self._is_valid_webhook_url(url) and self._wants_progress_updates(
                    url
                ):
                    webhook_id = str(uuid.uuid4())
                    delivery = WebhookDelivery(webhook_id, url, payload)
    ...
    async def _attempt_webhook_delivery(self, delivery: WebhookDelivery) -> bool:
        ...
        try:
            ...
            async with session.post(
                delivery.url, json=delivery.payload, headers=headers
            ) as response:
                ...
                if 200 <= response.status < 300:
                    ...
                    delivery.error_message = (
                        f"HTTP {response.status}: {delivery.response_body[:200]}"
                    )
                    return False
    ...
    async def get_delivery_status(self, webhook_id: str) -> dict[str, Any] | None:
