...
class EnterpriseRBACService:
    ...
    async def initialize_system_permissions(self, db: AsyncSession) -> None:
        ...
        try:
            ...
            permission_mappings = {
                ...
                Permission.CREDENTIALS_DELETE_ALL: (
                    "credentials",
                    "credential",
                    "delete_all",
                    True,
                ...
                Permission.ADMIN_SECURITY_CONFIG: (
                    "security",
                    "security",
                    "configure",
                    True,
    ...
    async def initialize_system_roles(self, db: AsyncSession) -> None:
        ...
        try:
            ...
            permissions_map = {
                p.permission_id: p for p in permissions_result.scalars().all()
            }
            ...
            for role_id, role_data in self.rbac_engine.roles.items():
                if role_id not in existing_roles:
                    ...
                    logger.info(
                        f"Created system role: {role_id} with {len(role_permissions)} permissions"
                    )
        ...
        try:
            ...
            result = await db.execute(
                select(EnterpriseRole).where(
                    EnterpriseRole.role_id == role_data.role_id
                )
            )
        ...
        try:
            ...
            tenant_result = await db.execute(
                select(EnterpriseTenant).where(
                    EnterpriseTenant.id == assignment.tenant_id
                )
            )
    ...
    async def check_user_permission(
        ...
        try:
            ...
            query = (
                select(EnterpriseRole)
                .join(
                    user_tenant_roles, EnterpriseRole.id == user_tenant_roles.c.role_id
                )
                .where(user_tenant_roles.c.user_id == user_id)
            )
