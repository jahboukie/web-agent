...
class TaskService:
    ...
    async def create_task(
        db: AsyncSession, user_id: int, task_data: TaskCreate
    ) -> Task:
        """
    ...
    async def get_task(db: AsyncSession, task_id: int, user_id: int) -> Task | None:
        ...
        try:
            ...
            if task:
                logger.debug(
                    "Task retrieved successfully", task_id=task_id, user_id=user_id
                )
            else:
                logger.warning(
                    "Task not found or access denied", task_id=task_id, user_id=user_id
                )
    ...
    async def get_user_tasks(
        ...
        try:
            ...
            logger.error("Failed to retrieve user tasks", user_id=user_id, error=str(e))
            return [], 0

    @staticmethod
    async def update_task(
        db: AsyncSession, task_id: int, user_id: int, task_data: TaskUpdate
    ) -> Task | None:
        """
        ...
        try:
            ...
            if not task:
                logger.warning(
                    "Task not found for update", task_id=task_id, user_id=user_id
                )
                return None
            ...
            if task.status in [TaskStatus.IN_PROGRESS]:
                logger.warning(
                    "Cannot update task in progress",
                    task_id=task_id,
                    status=task.status,
                )
            ...
            logger.info("Task updated successfully", task_id=task_id, user_id=user_id)
            ...
            await db.rollback()
            logger.error(
                "Failed to update task", task_id=task_id, user_id=user_id, error=str(e)
            )
    ...
    async def delete_task(db: AsyncSession, task_id: int, user_id: int) -> bool:
        ...
        try:
            ...
            if not task:
                logger.warning(
                    "Task not found for deletion", task_id=task_id, user_id=user_id
                )
                return False
            ...
            await db.rollback()
            logger.error(
                "Failed to delete task", task_id=task_id, user_id=user_id, error=str(e)
            )
