...
from app.schemas.analytics import (
    ...
    SuccessMetric, UserSubscription, ComponentMetrics, ROICalculation,
    SubscriptionTier, ComponentType, TrendDirection, ConversionMetrics
)
...
class AnalyticsService:
    ...
    async def get_dashboard_data(self, db: AsyncSession, user_id: int) -> AnalyticsDashboard:
        """
        ...
        subscription = await self.subscription_service.get_user_subscription(db, user_id)
    ...
    async def get_usage_metrics(self, db: AsyncSession, user_id: int, hours: int = 24) -> UsageMetrics:
        """
        ...
        plan_query = select(func.count(ExecutionPlan.id)).where(
            and_(ExecutionPlan.user_id == user_id, ExecutionPlan.created_at >= cutoff_time)
        )
        ...
        successful_tasks = (await db.execute(
            select(func.count(Task.id)).where(
                and_(Task.user_id == user_id, Task.status == "completed")
            )
        )).scalar() or 0
        
        parse_success_rate = (successful_tasks / total_tasks * 100) if total_tasks > 0 else 0
    ...
    async def get_upgrade_opportunities(self, db: AsyncSession, user_id: int) -> List[UpgradeOpportunity]:
        """
        ...
        if subscription.tier == SubscriptionTier.FREE:
            ...
            if parse_usage_pct > 80:
                opportunities.append(UpgradeOpportunity(
                    ...
                    days_until_limit=max(1, int((200 - usage_metrics.parses_used) / max(1, usage_metrics.parses_used / 30)))
                ))
    ...
    async def get_success_metrics(self, db: AsyncSession, user_id: int) -> List[SuccessMetric]:
        """
        Generate success metrics for value demonstration.
        """
    ...
    async def calculate_roi(self, db: AsyncSession, user_id: int, hourly_rate: float = 50.0) -> ROICalculation:
        """
        ...
        roi_percentage = (net_savings / max(subscription_cost, 1)) * 100 if subscription_cost > 0 else 0
        
        # Calculate payback period
        daily_savings = labor_cost_saved / 30
        payback_period_days = int(subscription_cost / max(daily_savings, 1)) if daily_savings > 0 else 0
    ...
    async def _generate_daily_usage_trend(self, db: AsyncSession, user_id: int, hours: int) -> List[Dict[str, Any]]:
    ...
    async def _calculate_avg_execution_time(self, db: AsyncSession, user_id: int) -> float:
        """Calculate average execution time."""
        return 15000.0  # Sample: 15 seconds

    async def get_platform_analytics(self, db: AsyncSession, user_id: int) -> PlatformAnalytics:
    ...
    async def get_conversion_metrics(self, db: AsyncSession, user_id: int) -> ConversionMetrics:
    ...
    async def get_component_metrics(self, db: AsyncSession, user_id: int,
                                  component_type: ComponentType, hours: int) -> ComponentMetrics:
    ...
    async def track_event(self, db: AsyncSession, user_id: int, event_type: str, event_data: dict):
