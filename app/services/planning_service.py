...
from app.langchain.tools.webpage_tools import (
...
class PlanningService:
    ...
    async def generate_plan_async(
        ...
        try:
            ...
            source_task, webpage_data = await self._get_source_data(
                db, task_id, user_id
            )
            ...
            execution_plan.validation_passed = (
                validation_result.get("overall_status") == "approved"
            )
            execution_plan.validation_warnings = validation_result.get(
                "findings", {}
            ).get("warnings", [])
            execution_plan.validation_errors = validation_result.get(
                "findings", {}
            ).get("critical_issues", [])
            execution_plan.plan_quality_score = validation_result.get("scores", {}).get(
            ...
            if (
                ...
                or validation_result.get("overall_status") != "approved"
                or validation_result.get("approval_status", {}).get(
    ...
    async def _get_source_data(
        ...
        if not webpage_data or "web_page" not in webpage_data:
            raise ValueError(
                "Source task does not contain valid webpage parsing results"
            )
    ...
    async def _prepare_agent_context(
        ...
        context = {
            ...
            "webpage_summary": self._create_webpage_summary(
                web_page, interactive_elements, content_blocks
            ),
        }
    ...
    def _create_webpage_summary(
        ...
        if element_types:
            element_summary = ", ".join(
                [f"{count} {type}" for type, count in element_types.items()]
            )
    ...
    async def _execute_planning_workflow(
        self, agent_context: dict[str, Any], planning_options: dict[str, Any]
    ) -> dict[str, Any]:
    ...
    async def _parse_agent_output(
        ...
        try:
            ...
            execution_plan = ExecutionPlan(
                ...
                requires_sensitive_actions=plan_data.get(
    ...
    async def validate_plan(
        self, execution_plan: ExecutionPlan, webpage_data: dict[str, Any] = None
    ) -> dict[str, Any]:
        """
        ...
        try:
            ...
            validation_result = await self.plan_validator.validate_execution_plan(
                plan_dict, webpage_data or execution_plan.source_webpage_data
            )
            ...
            logger.error(
                "Plan validation failed", plan_id=execution_plan.id, error=str(e)
            )
            return {
    ...
    async def get_planning_metrics(
        ...
        try:
            ...
            avg_generation_time = (
                sum(p.planning_duration_ms or 0 for p in plans) / total_plans
            )
            ...
            return {
                ...
                "approval_rate": (
                    round(approved_plans / total_plans, 3) if total_plans > 0 else 0.0
                ),
