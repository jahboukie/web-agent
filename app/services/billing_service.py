...
from app.schemas.analytics import BillingInfo, UserSubscription
...
class BillingService:
    ...
    async def create_checkout_session(self, db: AsyncSession, user_id: int, 
                                    target_tier: str, billing_cycle: str = "monthly") -> Dict[str, str]:
        """
        ...
        if billing_cycle == "annual":
            annual_discount = self.subscription_service.PRICING_TIERS[tier].get("annual_discount", 0.0)
    ...
    async def handle_successful_payment(self, db: AsyncSession, user_id: int, 
                                      session_id: str, target_tier: str) -> bool:
        """
    ...
    async def _send_upgrade_confirmation(self, user_id: int, target_tier: str):
        ...
        logger.info("Sending upgrade confirmation", 
                   user_id=user_id,
                   target_tier=target_tier)
    ...
    async def calculate_proration(self, db: AsyncSession, user_id: int, 
                                target_tier: str) -> Dict[str, float]:
        """
        Calculate proration for mid-cycle upgrades.
        """
        
        current_subscription = await self.subscription_service.get_user_subscription(db, user_id)
        
        # Calculate days remaining in current period
        now = datetime.utcnow()
        days_remaining = (current_subscription.current_period_end - now).days
        days_in_period = (current_subscription.current_period_end - current_subscription.current_period_start).days
        ...
        target_daily_rate = self.subscription_service.get_tier_cost(target_tier_enum) / days_in_period
    ...
    async def get_usage_overage_charges(self, db: AsyncSession, user_id: int) -> Dict[str, float]:
        """
    ...
    async def apply_discount_code(self, db: AsyncSession, user_id: int, 
        ...
        discount_codes = {
            "WELCOME20": {"type": "percentage", "value": 0.20, "description": "20% off first month"},
            "SAVE50": {"type": "fixed", "value": 50.0, "description": "$50 credit"},
            "ANNUAL15": {"type": "percentage", "value": 0.15, "description": "15% off annual plans"}
        }
        ...
        logger.info("Applying discount code", 
                   user_id=user_id,
