...
from app.schemas.enterprise import ABACPolicyCreate
from app.security.rbac_engine import AccessDecision
...
class ABACPolicyEngine:
    ...
    async def create_policy(
        self, db: AsyncSession, policy_data: ABACPolicyCreate, created_by: int
    ) -> ABACPolicy:
        ...
        try:
            ...
            if result.scalar_one_or_none():
                raise ValueError(
                    f"Policy with ID '{policy_data.policy_id}' already exists"
                )
    ...
    def _matches_resource_pattern(
        self, resource_patterns: list[str], resource_type: str
    ) -> bool:
    ...
    async def _get_user_attributes(
        ...
        try:
            ...
            return {
                ...
                "user.trust_score": (
                    user.trust_score.get("current", 1.0) if user.trust_score else 1.0
                ),
                "user.risk_profile": (
                    user.risk_profile.get("level", "low")
        ...
        try:
            ...
            if settings.ENABLE_ZERO_TRUST and context.get("access_context"):
                try:
                    ...
                    trust_assessment = (
                        await self.zero_trust_engine.calculate_trust_score(
    ...
    async def _evaluate_policy_conditions(
        self, policy: ABACPolicy, attributes: dict[str, Any]
    ) -> bool:
        ...
        for attr_name, condition in conditions.items():
            ...
            for operator in condition.keys():
                if operator not in valid_operators:
                    raise ValueError(
                        f"Invalid operator '{operator}' in condition for '{attr_name}'"
                    )
