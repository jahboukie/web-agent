...
class WebhookConfigRequest(BaseModel):
    ...
    def validate_events(cls, v):
        valid_events = ["execution_completed", "execution_progress", "execution_started"]
        for event in v:
...
class WebhookTestResponse(BaseModel):
    ...
    response_body: Optional[str] = Field(
        default=None,
        description="HTTP response body (truncated)"
    )
...
class WebhookDeliveryStatus(BaseModel):
    ...
    response_status: Optional[int] = Field(
        default=None,
        description="HTTP response status from last attempt"
    )
