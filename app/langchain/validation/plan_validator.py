...
class PlanValidator:
    ...
    async def validate_execution_plan(
        ...
        try:
            ...
            await self._validate_feasibility(execution_plan, webpage_data, validation_result)
            ...
            return {
                ...
                'validation_timestamp': datetime.utcnow().isoformat(),
                'scores': {'safety_score': 0.0, 'feasibility_score': 0.0, 'quality_score': 0.0},
                'findings': {'critical_issues': [f"Validation error: {str(e)}"]},
                'approval_status': {'requires_human_approval': True, 'risk_level': 'high'}
            }
    
    async def _validate_safety(self, execution_plan: Dict[str, Any], result: Dict[str, Any]) -> None:
        ...
        if plan_meta.get('requires_sensitive_actions', False):
            warnings.append("Plan involves sensitive actions that require careful review")
            safety_score -= 0.2
        ...
        for step in action_steps:
            ...
            input_value = step.get('input_value', '').lower() if step.get('input_value') else ''
            
            # Check for dangerous patterns
            for pattern in self.dangerous_patterns:
                if (re.search(pattern, step_description) or 
                    re.search(pattern, step_name) or 
                    re.search(pattern, input_value)):
                    safety_issues.append(f"Step {step.get('step_number')}: Contains potentially dangerous action pattern")
                    safety_score -= 0.5
            ...
            for pattern in self.sensitive_patterns:
                if (re.search(pattern, step_description) or 
                    re.search(pattern, step_name) or 
                    re.search(pattern, input_value)):
                    warnings.append(f"Step {step.get('step_number')}: Involves sensitive action requiring approval")
                    safety_score -= 0.1
            
            # Check for irreversible actions without confirmation
            if (step.get('is_critical', False) and 
                not step.get('requires_confirmation', False) and
                any(word in step_description for word in ['delete', 'remove', 'clear', 'submit'])):
                warnings.append(f"Step {step.get('step_number')}: Critical action without confirmation")
                safety_score -= 0.2
        ...
        if safety_score > 0.8:
            result['findings']['positive_aspects'].append("Plan demonstrates good safety practices")
    ...
    async def _validate_feasibility(
        ...
        element_selectors = {elem.get('selector', ''): elem for elem in interactive_elements}
        ...
        for step in action_steps:
            ...
            if target_selector and target_selector not in element_selectors:
                ...
                for selector in element_selectors.keys():
                    if target_selector in selector or selector in target_selector:
                        warnings.append(f"Step {step_num}: Target selector may need adjustment")
                        feasibility_score -= 0.1
                        similar_found = True
                        break
                
                if not similar_found:
                    feasibility_issues.append(f"Step {step_num}: Target element not found on page")
            ...
            if confidence < 0.5:
                warnings.append(f"Step {step_num}: Low confidence score ({confidence:.2f})")
                feasibility_score -= 0.1
            elif confidence < 0.3:
                feasibility_issues.append(f"Step {step_num}: Very low confidence score ({confidence:.2f})")
                feasibility_score -= 0.2
        ...
        if feasibility_score > 0.8:
            result['findings']['positive_aspects'].append("Plan shows high feasibility with available elements")
    
    async def _validate_quality(self, execution_plan: Dict[str, Any], result: Dict[str, Any]) -> None:
        ...
        if not action_steps:
            ...
            for step in action_steps:
                if step.get('step_number') != expected_step:
                    warnings.append(f"Step numbering inconsistency at step {expected_step}")
                    quality_score -= 0.1
                expected_step += 1
            ...
            for step in action_steps:
                ...
                if timeout < 5 or timeout > 300:
                    warnings.append(f"Step {step_num}: Unusual timeout value ({timeout}s)")
                    quality_score -= 0.05
        ...
        if not has_error_handling:
            recommendations.append("Consider adding error handling and fallback strategies")
            quality_score -= 0.1
        ...
        if quality_score > 0.8:
            result['findings']['positive_aspects'].append("Plan demonstrates high quality structure")
    ...
    def _is_action_compatible(self, action_type: str, element_type: str) -> bool:
        ...
        return 'any' in compatible_elements or element_type.lower() in compatible_elements
    ...
    async def _calculate_overall_scores(self, result: Dict[str, Any]) -> None:
        ...
        if scores['safety_score'] < 0.5 or len(result['findings']['critical_issues']) > 0:
    ...
    async def _determine_approval_status(self, result: Dict[str, Any]) -> None:
        ...
        # Require approval for sensitive actions
            ...
            approval_status['approval_reason'] = 'Plan meets safety and quality standards'
