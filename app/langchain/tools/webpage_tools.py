...
class WebpageAnalysisTool(WebAgentBaseTool):
    ...
    def _execute_tool(self, analysis_request: str, **kwargs) -> str:
        ...
        if "structure" in analysis_request_lower or "layout" in analysis_request_lower:
            ...
            "interactive" in analysis_request_lower
            or "element" in analysis_request_lower
        ):
    ...
    def _analyze_forms(self) -> str:
        ...
        input_fields = [
            e for e in form_elements if e.get("type") in ["input", "textarea", "select"]
        ]
    ...
    def _analyze_navigation(self) -> str:
        ...
        for elem in self.webpage_data.get("interactive_elements", []):
            ...
            if (
                elem_type == "link"
                or "nav" in elem_text
                or "menu" in elem_text
                or "nav" in elem_label
                or "menu" in elem_label
            ):
    ...
    def _analyze_interactive_elements(self) -> str:
        ...
        for elem_type, elems in element_groups.items():
            ...
            for elem in elems[:5]:  # Show first 5 of each type
                text = elem.get(
                    "text", elem.get("label", elem.get("placeholder", "No text"))
                )
    ...
    def _analyze_general(self) -> str:
        ...
        if elements:
            high_confidence_elements = [
                e for e in elements if e.get("confidence", 0) > 0.8
            ]
            analysis.append("\nAutomation Readiness:")
            analysis.append(
                f"- High confidence elements: {len(high_confidence_elements)}/{len(elements)}"
            )
            analysis.append(
                f"- Automation feasibility: {'High' if len(high_confidence_elements) > len(elements) * 0.7 else 'Medium' if len(high_confidence_elements) > len(elements) * 0.4 else 'Low'}"
            )
...
class ElementInspectorTool(WebAgentBaseTool):
    ...
    def _element_matches_query(self, element: dict[str, Any], query: str) -> bool:
        ...
        if "submit" in query and (
            elem_type == "button" and "submit" in element.get("text", "").lower()
        ):
            return True
        if "login" in query and (
            "login" in element.get("text", "").lower()
...
class ActionCapabilityAssessor(WebAgentBaseTool):
    ...
    def _execute_tool(self, goal_description: str, **kwargs) -> str:
        ...
        for action, elements in element_assessment.items():
            if elements:
                ...
                assessment.append(
                    f"  - {action}: ✓ Available (confidence: {confidence:.2f})"
                )
                assessment.append(
                    f"    Best match: {best_element.get('text', best_element.get('type', 'Unknown'))}"
                )
            else:
        ...
        assessment.append(
            f"Feasibility: {'High' if overall_confidence > 0.8 else 'Medium' if overall_confidence > 0.5 else 'Low'}"
        )

        # Provide recommendations
        recommendations = self._generate_recommendations(
            overall_confidence, element_assessment
        )
        if recommendations:
            assessment.append("\nRecommendations:")
    ...
    def _analyze_goal_requirements(self, goal: str) -> list[str]:
        ...
        if any(word in goal_lower for word in ["login", "sign in", "authenticate"]):
            required_actions.extend(
                ["find_email_input", "find_password_input", "find_login_button"]
            )
        ...
        if any(
            ...
            required_actions.extend(
                ["find_email_input", "find_password_input", "find_signup_button"]
            )
    ...
    def _assess_element_availability(
        ...
        for action in required_actions:
            ...
            if action == "find_email_input":
                matching_elements = [
                    e
                    for e in elements
                    if e.get("input_type") == "email"
                    or "email" in e.get("placeholder", "").lower()
                ]

            elif action == "find_password_input":
                matching_elements = [
                    e for e in elements if e.get("input_type") == "password"
                ]
                ...
                matching_elements = [
                    ...
                    and any(
                        word in e.get("text", "").lower()
                        for word in ["login", "sign in", "log in"]
                    )
                ]
                ...
                matching_elements = [
                    ...
                    and any(
                        word in e.get("text", "").lower()
                        for word in ["sign up", "register", "create"]
                    )
                ]
                ...
                matching_elements = [
                    ...
                    for e in elements
                    if e.get("type") == "input"
                    and "search" in e.get("placeholder", "").lower()
                ]
                ...
                matching_elements = [
                    ...
                    for e in elements
                    if e.get("type") == "button"
                    and "search" in e.get("text", "").lower()
                ]
    ...
    def _generate_recommendations(
        self, confidence: float, element_assessment: dict[str, list[dict[str, Any]]]
    ) -> list[str]:
        ...
        if confidence < 0.3:
            recommendations.append(
                "Low automation confidence. Consider manual verification of element selectors."
            )
            recommendations.append(
                "Review page structure and identify alternative automation approaches."
            )

        elif confidence < 0.7:
            recommendations.append(
                "Medium automation confidence. Test thoroughly before deployment."
            )
            recommendations.append(
                "Consider adding fallback strategies for unreliable elements."
            )

        else:
            recommendations.append(
                "High automation confidence. Proceed with plan generation."
            )

        # Check for missing critical elements
        missing_actions = [
            action for action, elements in element_assessment.items() if not elements
        ]
        if missing_actions:
            recommendations.append(
                f"Missing elements for: {', '.join(missing_actions)}"
            )
            recommendations.append(
                "Consider alternative selectors or manual element identification."
            )
