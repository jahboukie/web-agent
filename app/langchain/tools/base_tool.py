...
class WebAgentBaseTool(BaseTool, ABC):
    ...
    def __init__(self, webpage_data: Optional[Dict[str, Any]] = None, **kwargs):
        """
        ...
        object.__setattr__(self, '_logger', structlog.get_logger(self.__class__.__name__))
    ...
    def _run(self, query: str, **kwargs) -> str:
        ...
        try:
            ...
            self.logger.info("Tool execution completed", tool=self.name, result_length=len(str(result)))
            return result
    ...
    def _get_interactive_elements_by_type(self, element_type: str) -> list:
        ...
        return [
            elem for elem in self.webpage_data['interactive_elements']
        ...
        for elem in self.webpage_data['interactive_elements']:
            ...
            if partial_match:
                if (search_text in element_text or 
                    search_text in element_label or 
                    search_text in element_placeholder):
                    matching_elements.append(elem)
            else:
                if (search_text == element_text or 
                    search_text == element_label or 
                    search_text == element_placeholder):
                    matching_elements.append(elem)
