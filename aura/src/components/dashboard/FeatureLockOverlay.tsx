...
import { Lock, Crown, Star, Shield, Zap, ArrowRight, Sparkles } from 'lucide-react';
import { cn } from '../../lib/utils';

export interface FeatureLockOverlayProps {
  featureName: string;
  requiredPlan: 'reader_pro' | 'planner_pro' | 'actor_pro' | 'complete' | 'enterprise';
  currentPlan?: string;
  onUpgradeClick: () => void;
  className?: string;
...
const planInfo = {
  reader_pro: {
    ...
    benefits: ['Unlimited website parsing', 'Advanced element detection', 'Performance analytics'],
  },
  planner_pro: {
    name: 'Planner Pro', 
    price: '$179/mo',
    ...
    benefits: ['Unlimited AI planning', 'Confidence scoring', 'Workflow optimization'],
  },
  actor_pro: {
    name: 'Actor Pro',
    price: '$229/mo', 
  ...
  complete: {
    ...
    benefits: ['All tools included', 'Cross-platform analytics', 'Priority support', 'Save 40%'],
  },
  enterprise: {
    ...
    benefits: ['White-label branding', 'Dedicated CSM', 'SLA guarantees', 'Compliance tools'],
  },
};
      ...
      <div className={cn(
        ...
        <div className="text-center max-w-md">
          ...
          <div className={cn(
            ...
            style === 'gradient' ? 'bg-white/20' : `bg-gradient-to-br ${plan.color}`
          )}>
            <Lock className={cn(
              'h-8 w-8',
          ...
          <h3 className={cn(
            ...
            style === 'gradient' ? 'text-white' : 'text-gray-900 dark:text-white'
          )}>
          ...
          <div className={cn(
            ...
            style === 'gradient' ? 'text-white/90' : 'text-gray-600 dark:text-gray-300'
          )}>
              ...
              <ul className="space-y-2">
                {plan.benefits.slice(0, 3).map((benefit, index) => (
                  <li key={index} className={cn(
                    ...
                    style === 'gradient' ? 'text-white/80' : 'text-gray-600 dark:text-gray-300'
                  )}>
          ...
          <p className={cn(
            'text-xs mt-3',
            style === 'gradient' ? 'text-white/70' : 'text-gray-500 dark:text-gray-400'
          )}>
            {requiredPlan === 'complete' && 'Save 40% with Complete Platform'}
            {requiredPlan === 'enterprise' && 'Includes dedicated customer success manager'}
            {!['complete', 'enterprise'].includes(requiredPlan) && 'Upgrade anytime • Cancel anytime'}
          </p>
        </div>
      </div>
    </div>
