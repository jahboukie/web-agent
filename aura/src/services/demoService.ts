...
const generateTrustAssessment = (user: any) => ({
  ...
  risk_factors:
    user.trust_score < 0.8 ? ["unusual_login_time", "new_device"] : [],
  required_actions:
    user.trust_score < 0.7 ? ["verify_device", "enable_mfa"] : [],
...
class DemoService {
  ...
  private async generateDemoToken(
    user: any,
    type: string = "access",
  ): Promise<string> {
    const header = {
