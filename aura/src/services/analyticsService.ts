...
class AnalyticsService {
  ...
  async getUpgradeOpportunities(): Promise<UpgradeOpportunity[]> {
    return apiService.get<UpgradeOpportunity[]>(
      "/analytics/upgrade-opportunities",
    );
  }
  ...
  async getROICalculation(hourlyRate: number = 50): Promise<any> {
    return apiService.get(
      `/analytics/roi-calculation?hourly_rate=${hourlyRate}`,
    );
  }
  ...
  private generateUpgradeOpportunities(
    ...
    if (subscription.tier === "free") {
      const parseUsage =
        subscription.usage.parses_used / (subscription.limits.parses as number);
      const planUsage =
        subscription.usage.plans_used / (subscription.limits.plans as number);
      const execUsage =
        subscription.usage.executions_used /
        (subscription.limits.executions as number);
      ...
      if (metrics.unified.total_workflows > 5) {
        opportunities.push({
          ...
          description:
            "You've completed 5+ workflows! See how Complete Platform users achieve 97% automation success.",
          urgency: "medium",
    ...
    if (
      ["reader_pro", "planner_pro", "actor_pro"].includes(subscription.tier)
    ) {
      opportunities.push({
        id: "bundle_savings",
        type: "feature_unlock",
        ...
        description:
          "Unlock all tools + unified analytics for less than what you're paying now.",
        urgency: "medium",
      ...
      upgrade_opportunities: [
        {
          ...
          description:
            "You've automated 6 workflows successfully. See how Complete Platform users achieve 97% success rates.",
          urgency: "medium",
