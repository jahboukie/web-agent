...
class WebAgentTester:
    ...
    async def test_user_registration(self) -> bool:
        ...
        try:
            response = await self.client.post(
                f"{BASE_URL}/api/v1/auth/register", json=TEST_USER
            )
    ...
    async def test_webpage_parsing(
        ...
        try:
            ...
            if response.status_code != 200:
                print(
                    f"❌ Failed to start parsing: {response.status_code} - {response.text}"
                )
    ...
    async def monitor_task_progress(self, task_id: int) -> dict[str, Any]:
        ...
        while time.time() - start_time < max_wait_time:
            try:
                ...
                print(
                    f"   Status: {status} | Progress: {progress}% | Step: {current_step}"
                )
    ...
    async def test_cache_performance(self) -> bool:
        ...
        if second_duration < first_duration * 0.5:  # At least 50% faster
            ...
            print(
                f"   Speed improvement: {((first_duration - second_duration) / first_duration * 100):.1f}%"
            )
            return True
        else:
    ...
    async def test_cache_stats(self) -> bool:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                print(
                    f"   Redis Memory: {cache_stats.get('redis_memory_used_mb', 0)} MB"
                )
                return True
            else:
