...
from app.security.zero_trust import DeviceFingerprint, TrustLevel
    ...
    try:
        ...
        if status:
            for provider_id, provider_status in status.items():
                health = (
                    "🟢 Healthy" if provider_status.get("healthy") else "🔴 Unhealthy"
                )
...
async def demo_integration_points():
    ...
    print("   - Zero Trust Engine: ✅ Integrated")
    print("   - Policy Evaluation: ✅ Enhanced with trust attributes")
