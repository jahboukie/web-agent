...
class DashboardFlowTester:
    ...
    async def test_analytics_dashboard_access(self):
        ...
        try:
            async with self.session.get(f"{API_BASE}/analytics/dashboard", headers=headers) as response:
                if response.status == 200:
                    data = await response.json()
            ...
            self.log_result("Analytics Dashboard", False, "Dashboard request failed", str(e))
            return None
    ...
    async def test_usage_metrics_api(self):
        ...
        try:
            async with self.session.get(f"{API_BASE}/analytics/usage", headers=headers) as response:
                if response.status == 200:
                    data = await response.json()
    ...
    async def test_upgrade_opportunities_api(self):
        ...
        try:
            async with self.session.get(f"{API_BASE}/analytics/upgrade-opportunities", headers=headers) as response:
                if response.status == 200:
                    data = await response.json()
                    
                    if isinstance(data, list):
                        self.log_result(
                            "Upgrade Opportunities", True,
            ...
            self.log_result("Upgrade Opportunities", False, "Upgrade opportunities request failed", str(e))
            return False
    ...
    async def test_complete_dashboard_workflow(self):
        ...
        self.log_result(
            "Complete Dashboard Workflow", workflow_success,
            "Full user workflow completed successfully" if workflow_success else "Workflow had failures"
        )
