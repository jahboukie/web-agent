...
class AnalyticsSystemValidator:
                ...
                async with self.session.get(f"{API_BASE}{endpoint}") as resp:
                    if resp.status == 200:
                        ...
                        if endpoint == "/analytics/dashboard":
                            required_fields = ["user_id", "subscription", "usage_metrics", "success_metrics"]
                            if all(field in data for field in required_fields):
                                print(f"      ✅ Dashboard structure valid")
                            else:
                                print(f"      ⚠️ Dashboard missing required fields")
    ...
    async def test_subscription_management(self):
        ...
        async with self.session.get(f"{API_BASE}/analytics/subscription") as resp:
            if resp.status == 200:
                ...
                print(f"   ✅ Current subscription: {subscription.get('tier', 'unknown')}")
                ...
                if all(field in subscription for field in required_fields):
                    ...
                    print(f"   📈 Usage: Parses {usage.get('parses_used', 0)}/{limits.get('parses', 'unlimited')}")
                    print(f"   📈 Usage: Plans {usage.get('plans_used', 0)}/{limits.get('plans', 'unlimited')}")
                    print(f"   📈 Usage: Executions {usage.get('executions_used', 0)}/{limits.get('executions', 'unlimited')}")
    ...
    async def test_upgrade_opportunities(self):
        ...
        async with self.session.get(f"{API_BASE}/analytics/upgrade-opportunities") as resp:
            if resp.status == 200:
    ...
    async def test_roi_calculation(self):
        ...
        for rate in hourly_rates:
            async with self.session.get(f"{API_BASE}/analytics/roi-calculation?hourly_rate={rate}") as resp:
                if resp.status == 200:
                    roi = await resp.json()
                    print(f"   ✅ ROI at ${rate}/hr:")
                    print(f"      Time Saved: {roi.get('time_saved_hours', 0):.1f} hours")
                    print(f"      Labor Cost Saved: ${roi.get('labor_cost_saved', 0):.0f}")
                    print(f"      ROI: {roi.get('roi_percentage', 0):.1f}%")
                    print(f"      Payback Period: {roi.get('payback_period_days', 0):.1f} days")
                else:
    ...
    async def test_analytics_tracking(self):
        ...
        events = [
            ...
            ("pricing_tier_selected", {"selected_tier": "complete_platform", "current_tier": "free"}),
        ...
        for event_type, event_data in events:
            try:
                ...
                async with self.session.post(f"{API_BASE}/analytics/track-event", json=payload) as resp:
                    if resp.status == 200:
                        print(f"   ✅ Event tracked: {event_type}")
                    else:
                        print(f"   ❌ Event tracking failed: {event_type} - {resp.status}")
    ...
    async def test_component_metrics(self):
        ...
        for component in components:
            async with self.session.get(f"{API_BASE}/analytics/component/{component}") as resp:
                if resp.status == 200:
                    ...
                    print(f"   ❌ {component.capitalize()} metrics failed: {resp.status}")
    ...
    async def test_billing_integration(self):
        ...
        async with self.session.get(f"{API_BASE}/analytics/billing") as resp:
            if resp.status == 200:
                ...
                print(f"   📅 Next Billing: {subscription.get('next_billing_date', 'N/A')}")
